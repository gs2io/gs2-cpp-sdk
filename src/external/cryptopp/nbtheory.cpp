...
bool TrialDivision(const Integer &p, unsigned bound)
{
	...
	GS2_CRYPTOPP_ASSERT(primeTable[primeTableSize-1] >= bound);
...
bool IsFermatProbablePrime(const Integer &n, const Integer &b)
{
	...
	GS2_CRYPTOPP_ASSERT(n>3 && b>1 && b<n-1);
...
bool PrimeSieve::NextCandidate(Integer &c)
{
	...
	GS2_CRYPTOPP_UNUSED(safe); GS2_CRYPTOPP_ASSERT(safe);
...
void PrimeSieve::DoSieve()
{
	...
	{
		GS2_CRYPTOPP_ASSERT(m_step%2==0);
...
{
	GS2_CRYPTOPP_ASSERT(!equiv.IsNegative() && equiv < mod);
	...
	GS2_CRYPTOPP_ASSERT(p > primeTable[primeTableSize-1]);
...
{
	GS2_CRYPTOPP_ASSERT(p < q*q*q);
	GS2_CRYPTOPP_ASSERT(p % q == 1);
...
// which holds iff p is built up of two factors (exercise ... )
	...
	GS2_CRYPTOPP_ASSERT(primeTableSize >= 50);
	for (int i=0; i<50; i++)
	{
...
Integer ModularSquareRoot(const Integer &a, const Integer &p)
{
	...
	GS2_CRYPTOPP_ASSERT(x.Squared()%p == a);
	return x;
}
...
bool SolveModularQuadraticEquation(Integer &r1, Integer &r2, const Integer &a, const Integer &b, const Integer &c, const Integer &p)
{
	...
	{
	default:
		GS2_CRYPTOPP_ASSERT(false);	// not reached
		return false;
	case -1:
		return false;
	case 0:
		r1 = r2 = (-b*(a+a).InverseMod(p)) % p;
		GS2_CRYPTOPP_ASSERT(((r1.Squared()*a + r1*b + c) % p).IsZero());
		return true;
	case 1:
		...
		r2 = (-s-b)*t % p;
		GS2_CRYPTOPP_ASSERT(((r1.Squared()*a + r1*b + c) % p).IsZero());
		GS2_CRYPTOPP_ASSERT(((r2.Squared()*a + r2*b + c) % p).IsZero());
		return true;
	}
}
...
{
	GS2_CRYPTOPP_ASSERT(b.Positive());
...
void PrimeAndGenerator::Generate(signed int delta, RandomNumberGenerator &rng, unsigned int pbits, unsigned int qbits)
{
	...
	GS2_CRYPTOPP_ASSERT(qbits > 4);
	GS2_CRYPTOPP_ASSERT(pbits > qbits);
	...
	if (qbits+1 == pbits)
	{
		...
		while (!success)
		{
			...
			while (sieve.NextCandidate(p))
			{
				GS2_CRYPTOPP_ASSERT(IsSmallPrime(p) || SmallDivisorsTest(p));
				q = (p-delta) >> 1;
				GS2_CRYPTOPP_ASSERT(IsSmallPrime(q) || SmallDivisorsTest(q));
		...
		if (delta == 1)
		{
			...
			GS2_CRYPTOPP_ASSERT((p%8==1 || p%8==7) ? g==2 : (p%12==1 || p%12==11) ? g==3 : g==4);
		}
		else
		{
			GS2_CRYPTOPP_ASSERT(delta == -1);
	...
	else
	{
		...
		if (delta==1)
		{
			...
			GS2_CRYPTOPP_ASSERT(a_exp_b_mod_c(g, q, p)==1);
		}
		else
		{
			GS2_CRYPTOPP_ASSERT(delta==-1);
			...
			GS2_CRYPTOPP_ASSERT(Lucas(q, g, p) == 2);
		}
	}
}
