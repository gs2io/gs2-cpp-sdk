...
#ifndef GS2_CRYPTOPP_SECKEY_H
#define GS2_CRYPTOPP_SECKEY_H
...
public:
	...
	GS2_CRYPTOPP_STATIC_CONSTEXPR size_t GS2_CRYPTOPP_API StaticGetValidBlockSize(size_t blocksize)
	{
		return GS2_CRYPTOPP_UNUSED(blocksize), static_cast<size_t>(BLOCKSIZE);
	}
	...
	/// \details The default implementation returns BLOCKSIZE. blocksize is unused
	///   in the default implementation.
	GS2_CRYPTOPP_STATIC_CONSTEXPR size_t GS2_CRYPTOPP_API StaticGetValidBlockSize(size_t keylength, size_t blocksize)
	{
		return GS2_CRYPTOPP_UNUSED(keylength), GS2_CRYPTOPP_UNUSED(blocksize), static_cast<size_t>(BLOCKSIZE);
	}
};
...
public:
	...
	GS2_CRYPTOPP_CONSTANT(ROUNDS = R)
};
...
public:
	...
	GS2_CRYPTOPP_STATIC_CONSTEXPR unsigned int StaticGetDefaultRounds(size_t keylength)
	{
		return GS2_CRYPTOPP_UNUSED(keylength), static_cast<unsigned int>(DEFAULT_ROUNDS);
	}
...
public:
	...
	GS2_CRYPTOPP_CONSTANT(KEYLENGTH=N)
	...
	GS2_CRYPTOPP_CONSTANT(MIN_KEYLENGTH=N)
	...
	GS2_CRYPTOPP_CONSTANT(IV_LENGTH = IV_L)
	...
	///   in the default implementation.
	GS2_CRYPTOPP_STATIC_CONSTEXPR size_t GS2_CRYPTOPP_API StaticGetValidKeyLength(size_t keylength)
	{
		return GS2_CRYPTOPP_UNUSED(keylength), static_cast<size_t>(KEYLENGTH);
	}
};
...
class VariableKeyLength
{
	...
	GS2_CRYPTOPP_COMPILE_ASSERT(Q > 0);
	GS2_CRYPTOPP_COMPILE_ASSERT(N % Q == 0);
	GS2_CRYPTOPP_COMPILE_ASSERT(M % Q == 0);
	GS2_CRYPTOPP_COMPILE_ASSERT(N < M);
	GS2_CRYPTOPP_COMPILE_ASSERT(D >= N);
	GS2_CRYPTOPP_COMPILE_ASSERT(M >= D);
...
public:
	...
	/// \brief The maximum key length used by the algorithm provided as a constant
	/// \details MAX_KEYLENGTH is provided in bytes, not bits
	GS2_CRYPTOPP_CONSTANT(MAX_KEYLENGTH=M)
	/// \brief The default key length used by the algorithm provided as a constant
	/// \details DEFAULT_KEYLENGTH is provided in bytes, not bits
	GS2_CRYPTOPP_CONSTANT(DEFAULT_KEYLENGTH=D)
	/// \brief The key length multiple used by the algorithm provided as a constant
	/// \details MAX_KEYLENGTH is provided in bytes, not bits
	GS2_CRYPTOPP_CONSTANT(KEYLENGTH_MULTIPLE=Q)
	/// \brief The default IV requirements for the algorithm provided as a constant
	/// \details The default value is NOT_RESYNCHRONIZABLE. See IV_Requirement
	///  in cryptlib.h for allowed values.
	GS2_CRYPTOPP_CONSTANT(IV_REQUIREMENT=IV_REQ)
...
public:
	/// \brief The minimum key length used by the algorithm provided as a constant
	/// \details MIN_KEYLENGTH is provided in bytes, not bits
	GS2_CRYPTOPP_CONSTANT(MIN_KEYLENGTH=T::MIN_KEYLENGTH)
	/// \brief The maximum key length used by the algorithm provided as a constant
	/// \details MIN_KEYLENGTH is provided in bytes, not bits
	GS2_CRYPTOPP_CONSTANT(MAX_KEYLENGTH=T::MAX_KEYLENGTH)
	/// \brief The default key length used by the algorithm provided as a constant
	/// \details MIN_KEYLENGTH is provided in bytes, not bits
	GS2_CRYPTOPP_CONSTANT(DEFAULT_KEYLENGTH=T::DEFAULT_KEYLENGTH)
	/// \brief The default IV requirements for the algorithm provided as a constant
	/// \details The default value is NOT_RESYNCHRONIZABLE. See IV_Requirement
	///  in cryptlib.h for allowed values.
	GS2_CRYPTOPP_CONSTANT(IV_REQUIREMENT=IV_REQ)
...
class GS2_CRYPTOPP_NO_VTABLE SimpleKeyingInterfaceImpl : public BASE
{
public:
...
class GS2_CRYPTOPP_NO_VTABLE BlockCipherImpl : public AlgorithmImpl<SimpleKeyingInterfaceImpl<TwoBases<BASE, INFO> > >
{
public:
