...
template <class W, bool T_64bit>
void BLAKE2_Base<W, T_64bit>::UncheckedSetKey(const byte *key, unsigned int length, const gs2::detail::CryptoPP::NameValuePairs& params)
{
    if (key && length)
    {
...
{
    GS2_CRYPTOPP_ASSERT(keyLength <= MAX_KEYLENGTH);
    GS2_CRYPTOPP_ASSERT(digestSize <= DIGESTSIZE);
    GS2_CRYPTOPP_ASSERT(saltLength <= SALTSIZE);
    GS2_CRYPTOPP_ASSERT(personalizationLength <= PERSONALIZATIONSIZE);
...
void BLAKE2_Base<W, T_64bit>::Update(const byte *input, size_t length)
{
    ...
    if (input && length)
    {
        GS2_CRYPTOPP_ASSERT(length <= BLOCKSIZE - state.length);
