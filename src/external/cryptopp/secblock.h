...
#ifndef GS2_CRYPTOPP_SECBLOCK_H
#define GS2_CRYPTOPP_SECBLOCK_H
...
public:
	...
	void destroy(pointer p) {GS2_CRYPTOPP_UNUSED(p); p->~T();}
...
#elif defined(GS2_CRYPTOPP_CXX11_ENUM)
	...
	GS2_CRYPTOPP_CONSTEXPR size_type max_size() const {return ELEMS_MAX;}

#if defined(GS2_CRYPTOPP_CXX11_VARIADIC_TEMPLATES) || defined(GS2_CRYPTOPP_DOXYGEN_PROCESSING)
	...
	/// \details This is a C++11 feature. It is available when GS2_CRYPTOPP_CXX11_VARIADIC_TEMPLATES
...
#define GS2_CRYPTOPP_INHERIT_ALLOCATOR_TYPES	\
...
{
	GS2_CRYPTOPP_ASSERT((oldPtr && oldSize) || !(oldPtr || oldSize));
	if (oldSize == newSize)
		return oldPtr;
...
///    GS2_CRYPTOPP_BOOL_ALIGN16. GS2_CRYPTOPP_BOOL_ALIGN16 is often used as the template parameter.
template <class T, bool T_Align16 = false>
class AllocatorWithCleanup : public AllocatorBase<T>
{
public:
	GS2_CRYPTOPP_INHERIT_ALLOCATOR_TYPES
	...
	void deallocate(void *ptr, size_type size)
	{
		...
		// GS2_CRYPTOPP_ASSERT((ptr && size) || !(ptr || size));
...
GS2_CRYPTOPP_DLL_TEMPLATE_CLASS AllocatorWithCleanup<byte>;
GS2_CRYPTOPP_DLL_TEMPLATE_CLASS AllocatorWithCleanup<word16>;
GS2_CRYPTOPP_DLL_TEMPLATE_CLASS AllocatorWithCleanup<word32>;
GS2_CRYPTOPP_DLL_TEMPLATE_CLASS AllocatorWithCleanup<word64>;
#if defined(GS2_CRYPTOPP_WORD128_AVAILABLE)
GS2_CRYPTOPP_DLL_TEMPLATE_CLASS AllocatorWithCleanup<word128, true>; // for Integer
#endif
#if GS2_CRYPTOPP_BOOL_X86
GS2_CRYPTOPP_DLL_TEMPLATE_CLASS AllocatorWithCleanup<word, true>;	 // for Integer
#endif
...
public:
	...
	{
		GS2_CRYPTOPP_UNUSED(n); GS2_CRYPTOPP_UNUSED(unused);
		GS2_CRYPTOPP_ASSERT(false); return NULLPTR;
	}

	void deallocate(void *p, size_type n)
	{
		GS2_CRYPTOPP_UNUSED(p); GS2_CRYPTOPP_UNUSED(n);
		GS2_CRYPTOPP_ASSERT(false);
	}

	GS2_CRYPTOPP_CONSTEXPR size_type max_size() const {return 0;}
	//LCOV_EXCL_STOP
};
...
public:
	...
	{
		GS2_CRYPTOPP_ASSERT(IsAlignedOn(m_array, 8));
	...
	void deallocate(void *ptr, size_type size)
	{
		...
		{
			GS2_CRYPTOPP_ASSERT(size <= S);
			GS2_CRYPTOPP_ASSERT(m_allocated);
			m_allocated = false;
			SecureWipeArray((pointer)ptr, size);
		}
		else
	...
	GS2_CRYPTOPP_CONSTEXPR size_type max_size() const {return STDMAX(m_fallbackAllocator.max_size(), S);}
...
private:
	...
	T m_array[S];
#else
	T* GetAlignedArray() {return (GS2_CRYPTOPP_BOOL_ALIGN16 && T_Align16) ? (T*)(void *)(((byte *)m_array) + (0-(size_t)m_array)%16) : m_array;}
	GS2_CRYPTOPP_ALIGN_DATA(8) T m_array[(GS2_CRYPTOPP_BOOL_ALIGN16 && T_Align16) ? S+8/sizeof(T) : S];
#endif
...
public:
	...
	SecBlock(const SecBlock<T, A> &t)
		: m_mark(t.m_mark), m_size(t.m_size), m_ptr(m_alloc.allocate(t.m_size, NULLPTR)) {
			GS2_CRYPTOPP_ASSERT((!t.m_ptr && !m_size) || (t.m_ptr && m_size));
	...
	SecBlock(const T *ptr, size_type len)
		: m_mark(ELEMS_MAX), m_size(len), m_ptr(m_alloc.allocate(len, NULLPTR)) {
			GS2_CRYPTOPP_ASSERT((!m_ptr && !m_size) || (m_ptr && m_size));
			if (ptr && m_ptr)
	...
	SecBlock<T, A>& operator+=(const SecBlock<T, A> &t)
	{
		GS2_CRYPTOPP_ASSERT((!t.m_ptr && !t.m_size) || (t.m_ptr && t.m_size));
		if (t.m_size)
		{
...
inline bool operator==(const gs2::detail::CryptoPP::AllocatorWithCleanup<T, A>&, const gs2::detail::CryptoPP::AllocatorWithCleanup<V, B>&) {return (true);}
template<class T, bool A, class V, bool B>
inline bool operator!=(const gs2::detail::CryptoPP::AllocatorWithCleanup<T, A>&, const gs2::detail::CryptoPP::AllocatorWithCleanup<V, B>&) {return (false);}
...
NAMESPACE_BEGIN(std)
template <class T, class A>
inline void swap(gs2::detail::CryptoPP::SecBlock<T, A> &a, gs2::detail::CryptoPP::SecBlock<T, A> &b)
{
	a.swap(b);
}
...
inline gs2::detail::CryptoPP::AllocatorWithCleanup<_Tp2>&
__stl_alloc_rebind(gs2::detail::CryptoPP::AllocatorWithCleanup<_Tp1>& __a, const _Tp2*)
{
	return (gs2::detail::CryptoPP::AllocatorWithCleanup<_Tp2>&)(__a);
}
#endif
