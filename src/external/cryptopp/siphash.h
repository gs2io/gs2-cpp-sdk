...
#ifndef GS2_CRYPTOPP_SIPHASH_H
#define GS2_CRYPTOPP_SIPHASH_H
...
{
public:
	GS2_CRYPTOPP_STATIC_CONSTEXPR const char* StaticAlgorithmName() {return "SipHash";}
	GS2_CRYPTOPP_CONSTANT(DIGESTSIZE = (T_128bit ? 16 : 8))
};
...
public:
	...
	virtual size_t GetValidKeyLength(size_t keylength) const
		{GS2_CRYPTOPP_UNUSED(keylength); return SipHash_Info<T_128bit>::DEFAULT_KEYLENGTH;}
...
{
	GS2_CRYPTOPP_ASSERT((input && length) || !length);
	if (!length) return;
...
{
	GS2_CRYPTOPP_ASSERT(digest);      // Pointer is valid
