...
{
	GS2_CRYPTOPP_ASSERT(value==0 || reg.size()>0);
...
unsigned int PolynomialMod2::Parity() const
{
	...
	for (i=0; i<reg.size(); i++)
		temp ^= reg[i];
	return gs2::detail::CryptoPP::Parity(temp);
}
...
PolynomialMod2& PolynomialMod2::operator<<=(unsigned int n)
{
	...
	int x; GS2_CRYPTOPP_UNUSED(x);
	GS2_CRYPTOPP_ASSERT(SafeConvert(n,x));
#endif
...
GF2NT::GF2NT(unsigned int c0, unsigned int c1, unsigned int c2)
	...
	, result((word)0, m)
{
	GS2_CRYPTOPP_ASSERT(c0 > c1 && c1 > c2 && c2==0);
}
...
const GF2NT::Element& GF2NT::MultiplicativeInverse(const Element &a) const
{
	...
	b[0]=1;
	GS2_CRYPTOPP_ASSERT(a.reg.size() <= m_modulus.reg.size());
	...
	while (1)
	{
		...
		while (!t)
		{
			...
			if (c[bcLen-1])
				bcLen++;
			GS2_CRYPTOPP_ASSERT(bcLen <= m_modulus.reg.size());
	...
	while (k >= WORD_BITS)
	{
		...
		if (t1 < WORD_BITS)
			for (unsigned int j=0; j<WORD_BITS-t1; j++)
			{
				...
				GS2_CRYPTOPP_ASSERT(shift < WORD_BITS);
	...
	if (i==BitsToWords(t0)-1 && t0%WORD_BITS)
	{
		...
		if ((t0-t1)%WORD_BITS)
		{
			...
			if ((t0-t1)%WORD_BITS > t0%WORD_BITS)
				...
				GS2_CRYPTOPP_ASSERT(temp << (WORD_BITS - (t0-t1)%WORD_BITS) == 0);
		}
		else
			b[i-(t0-t1)/WORD_BITS] ^= temp;
	}
