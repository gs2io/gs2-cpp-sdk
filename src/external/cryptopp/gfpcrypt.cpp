...
{
	GS2_CRYPTOPP_UNUSED(rng), GS2_CRYPTOPP_UNUSED(recoverableMessage), GS2_CRYPTOPP_UNUSED(recoverableMessageLength);
	GS2_CRYPTOPP_UNUSED(messageEmpty), GS2_CRYPTOPP_UNUSED(hashIdentifier);
	GS2_CRYPTOPP_ASSERT(recoverableMessageLength == 0);
	GS2_CRYPTOPP_ASSERT(hashIdentifier.second == 0);
	...
	byte *representative, size_t representativeBitLength) const
{
	...
	GS2_CRYPTOPP_UNUSED(representative); GS2_CRYPTOPP_UNUSED(representativeBitLength);
...
{
	GS2_CRYPTOPP_UNUSED(reversible);
...
{
	GS2_CRYPTOPP_UNUSED(checkForGroupMembership);
...
void DL_GroupParameters_IntegerBased::AssignFrom(const NameValuePairs &source)
{
	AssignFromHelper(this, source)
		GS2_CRYPTOPP_SET_FUNCTION_ENTRY2(Modulus, SubgroupGenerator)
		GS2_CRYPTOPP_SET_FUNCTION_ENTRY(SubgroupOrder)
		;
}
