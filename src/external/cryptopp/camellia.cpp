...
#define SLOW_ROUND(lh, ll, rh, rl, kh, kl)	{							\
	...
	zr=	rotlConstant<1>(s1[GETBYTE(zr, 3)]) |							\
		...
		(s1[rotlConstant<1>(GS2_CRYPTOPP_GET_BYTE_AS_BYTE(zr, 1))] << 16) |	\
		(s1[GETBYTE(zr, 0)] << 8);										\
	zl=	(s1[GETBYTE(zl, 3)] << 24) |									\
		(rotlConstant<1>(s1[GETBYTE(zl, 2)]) << 16) |					\
		(rotrConstant<1>(s1[GETBYTE(zl, 1)]) << 8) |					\
		s1[rotlConstant<1>(GS2_CRYPTOPP_GET_BYTE_AS_BYTE(zl, 0))];			\
	zl ^= zr;															\
...
#define PREPARE_KS_ROUNDS			\
	...
	kwr = (word64(k2) << 32) | k3
#define KS_ROUND_0(i)							\
	GS2_CRYPTOPP_ASSERT(IsAlignedOn(CALC_ADDR(ks32, i+EFI(0)),GetAlignmentOf<word64>()));	\
	GS2_CRYPTOPP_ASSERT(IsAlignedOn(CALC_ADDR(ks32, i+EFI(1)),GetAlignmentOf<word64>()));	\
	*(word64*)(void*)CALC_ADDR(ks32, i+EFI(0)) = kwl;	\
	*(word64*)(void*)CALC_ADDR(ks32, i+EFI(1)) = kwr
#define KS_ROUND(i, r, which)																						\
	GS2_CRYPTOPP_ASSERT(IsAlignedOn(CALC_ADDR(ks32, i+EFI(r<64)),GetAlignmentOf<word64>()));	\
	GS2_CRYPTOPP_ASSERT(IsAlignedOn(CALC_ADDR(ks32, i+EFI(r>64)),GetAlignmentOf<word64>()));	\
