...
#if defined(GS2_CRYPTOPP_DEBUG) && !defined(GS2_CRYPTOPP_DOXYGEN_PROCESSING) && !defined(GS2_CRYPTOPP_IS_DLL)
#include "sha3.h"
#include "pssr.h"
NAMESPACE_BEGIN(gs2)
...
bool RSAFunction::GetVoidValue(const char *name, const std::type_info &valueType, void *pValue) const
{
	return GetValueHelper(this, name, valueType, pValue).Assignable()
		GS2_CRYPTOPP_GET_FUNCTION_ENTRY(Modulus)
		GS2_CRYPTOPP_GET_FUNCTION_ENTRY(PublicExponent)
		;
}
...
void RSAFunction::AssignFrom(const NameValuePairs &source)
{
	AssignFromHelper(this, source)
		GS2_CRYPTOPP_SET_FUNCTION_ENTRY(Modulus)
		GS2_CRYPTOPP_SET_FUNCTION_ENTRY(PublicExponent)
		;
}
...
void InvertibleRSAFunction::GenerateRandom(RandomNumberGenerator &rng, const NameValuePairs &alg)
{
	...
	GS2_CRYPTOPP_ASSERT(modulusSize >= 16);
	if (modulusSize < 16)
		throw InvalidArgument("InvertibleRSAFunction: specified modulus size is too small");

	m_e = alg.GetValueWithDefault(Name::PublicExponent(), Integer(17));

	GS2_CRYPTOPP_ASSERT(m_e >= 3); GS2_CRYPTOPP_ASSERT(!m_e.IsEven());
	if (m_e < 3 || m_e.IsEven())
	...
	GS2_CRYPTOPP_ASSERT(m_d.IsPositive());
...
bool InvertibleRSAFunction::GetVoidValue(const char *name, const std::type_info &valueType, void *pValue) const
{
	return GetValueHelper<RSAFunction>(this, name, valueType, pValue).Assignable()
		GS2_CRYPTOPP_GET_FUNCTION_ENTRY(Prime1)
		GS2_CRYPTOPP_GET_FUNCTION_ENTRY(Prime2)
		GS2_CRYPTOPP_GET_FUNCTION_ENTRY(PrivateExponent)
		GS2_CRYPTOPP_GET_FUNCTION_ENTRY(ModPrime1PrivateExponent)
		GS2_CRYPTOPP_GET_FUNCTION_ENTRY(ModPrime2PrivateExponent)
		GS2_CRYPTOPP_GET_FUNCTION_ENTRY(MultiplicativeInverseOfPrime2ModPrime1)
		;
}
...
void InvertibleRSAFunction::AssignFrom(const NameValuePairs &source)
{
	AssignFromHelper<RSAFunction>(this, source)
		GS2_CRYPTOPP_SET_FUNCTION_ENTRY(Prime1)
		GS2_CRYPTOPP_SET_FUNCTION_ENTRY(Prime2)
		GS2_CRYPTOPP_SET_FUNCTION_ENTRY(PrivateExponent)
		GS2_CRYPTOPP_SET_FUNCTION_ENTRY(ModPrime1PrivateExponent)
		GS2_CRYPTOPP_SET_FUNCTION_ENTRY(ModPrime2PrivateExponent)
		GS2_CRYPTOPP_SET_FUNCTION_ENTRY(MultiplicativeInverseOfPrime2ModPrime1)
		;
}
