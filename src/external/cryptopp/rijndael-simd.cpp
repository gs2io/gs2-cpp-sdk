...
// We set GS2_CRYPTOPP_POWER8_CRYPTO_AVAILABLE based on compiler version.
// If the crypto is not available, then we have to disable it here.
#if !(defined(__CRYPTO) || defined(_ARCH_PWR8) || defined(_ARCH_PWR9))
# undef GS2_CRYPTOPP_POWER8_CRYPTO_AVAILABLE
# undef GS2_CRYPTOPP_POWER8_AES_AVAILABLE
#endif
...
#if defined(GS2_CRYPTOPP_POWER8_AES_AVAILABLE)
# include "ppc-simd.h"
#endif
...
bool CPU_ProbeAES()
{
    ...
    return false;
#elif (GS2_CRYPTOPP_ARM_AES_AVAILABLE)
...
#else
    return false;
#endif  // GS2_CRYPTOPP_ARM_AES_AVAILABLE
}
...
#endif  // GS2_CRYPTOPP_AESNI_AVAILABLE
...
{
    GS2_CRYPTOPP_ASSERT(IsAlignedOn(subkeys, 16));
    const byte *keys = reinterpret_cast<const byte*>(subkeys);
...
#endif  // GS2_CRYPTOPP_POWER8_AES_AVAILABLE
NAMESPACE_END
NAMESPACE_END
NAMESPACE_END
