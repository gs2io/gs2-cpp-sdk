...
//            scoped_main added to gs2::detail::CryptoPP::Test namespace by JW in July 2017
//            Also see http://github.com/weidai11/cryptopp/issues/447

#define GS2_CRYPTOPP_DEFAULT_NO_DLL
...
void RegisterFactories(gs2::detail::CryptoPP::Test::TestClass suites);
...
int scoped_main(int argc, char *argv[])
{
	...
	try
	{
		...
		else if (command == "mac_dll")
		{
			...
			byte mac[] = GS2_CRYPTOPP_DUMMY_DLL_MAC;
			...
			GS2_CRYPTOPP_ASSERT(pMac->DigestSize() == sizeof(mac));
		...
		{
			// TestDataFile() adds GS2_CRYPTOPP_DATA_DIR as required
		...
		{
			FileSource usage(GS2_CRYPTOPP_DATA_DIR "TestData/usage.dat", true, new FileSink(std::cout));
			return 1;
		}
		else if (command == "V")
		{
			std::cout << GS2_CRYPTOPP_VERSION / 100 << '.' << (GS2_CRYPTOPP_VERSION % 100) / 10 << '.' << GS2_CRYPTOPP_VERSION % 10 << std::endl;
		}
		else
		{
...
void SecretShareFile(int threshold, int nShares, const char *filename, const char *seed)
{
	GS2_CRYPTOPP_ASSERT(nShares >= 1 && nShares<=1000);
...
void SecretRecoverFile(int threshold, const char *outFilename, char *const *inFilenames)
{
	GS2_CRYPTOPP_ASSERT(threshold >= 1 && threshold <=1000);
...
void InformationRecoverFile(int threshold, const char *outFilename, char *const *inFilenames)
{
	GS2_CRYPTOPP_ASSERT(threshold<=1000);
	if (threshold < 1 || threshold > 1000)
...
void ForwardTcpPort(const char *sourcePortName, const char *destinationHost, const char *destinationPortName)
{
	...
	GS2_CRYPTOPP_UNUSED(sourcePortName), GS2_CRYPTOPP_UNUSED(destinationHost), GS2_CRYPTOPP_UNUSED(destinationPortName);
...
int GS2_CRYPTOPP_API main(int argc, char *argv[])
{
	return gs2::detail::CryptoPP::Test::scoped_main(argc, argv);
}
