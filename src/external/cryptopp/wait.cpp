...
DWORD WINAPI WaitingThread(LPVOID lParam)
{
	...
	while (true)
	{
		...
		GS2_CRYPTOPP_ASSERT(result != WAIT_FAILED);
#else
		...
		{
			GS2_CRYPTOPP_ASSERT(!"error in WaitingThread");	// break here so we can see which thread has an error
