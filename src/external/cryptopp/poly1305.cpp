...
void Poly1305_Base<T>::UncheckedSetKey(const byte *key, unsigned int length, const NameValuePairs &params)
{
	...
	if (params.GetValue(Name::IV(), t) && t.begin() && t.size())
	{
		...
		GS2_CRYPTOPP_ASSERT(t.size() == m_nk.size());
...
{
	GS2_CRYPTOPP_ASSERT(mac);      // Pointer is valid
	GS2_CRYPTOPP_ASSERT(!m_used);  // Nonce is fresh
...
{
	GS2_CRYPTOPP_ASSERT(nonceLength == -1 || nonceLength == (int)BLOCKSIZE);
