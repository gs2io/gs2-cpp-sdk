...
void CFB_ModePolicy::Iterate(byte *output, const byte *input, CipherDir dir, size_t iterationCount)
{
	...
	GS2_CRYPTOPP_ASSERT(output);
	GS2_CRYPTOPP_ASSERT(m_cipher->IsForwardTransformation());	// CFB mode needs the "encrypt" direction of the underlying block cipher, even to decrypt
	GS2_CRYPTOPP_ASSERT(m_feedbackSize == BlockSize());
...
{
	GS2_CRYPTOPP_ASSERT(length == BlockSize());
...
{
	GS2_CRYPTOPP_ASSERT(m_cipher->IsForwardTransformation());	// OFB mode needs the "encrypt" direction of the underlying block cipher, even to decrypt
...
{
	GS2_CRYPTOPP_ASSERT(m_cipher->IsForwardTransformation());	// CTR mode needs the "encrypt" direction of the underlying block cipher, even to decrypt
	unsigned int s = BlockSize();
