...
if [[ -z "$CPU_FREQ" ]]; then
	if [[ ! -z "GS2_CRYPTOPP_CPU_SPEED" ]]; then
		CPU_FREQ="$GS2_CRYPTOPP_CPU_SPEED"
	else
		CPU_FREQ=2.8
	fi
fi
...
if [[ -z "$OPT" ]]; then
	...
	"$CXX" -DGS2_CRYPTOPP_ADHOC_MAIN -O3 adhoc.cpp -o "$TMP/adhoc.exe" &>/dev/null
	if [[ ("$?" -eq "0") ]]; then
		OPT=-O3
	fi
fi
...
if [[ -z "$OPT" ]]; then
	rm -f "$TMP/adhoc.exe" &>/dev/null
	"$CXX" -DGS2_CRYPTOPP_ADHOC_MAIN -xO3 adhoc.cpp -o "$TMP/adhoc.exe" &>/dev/null
	if [[ ("$?" -eq "0") ]]; then
		OPT=-xO3
	fi
fi
...
if [[ -z "$OPT" ]]; then
	rm -f "$TMP/adhoc.exe" &>/dev/null
	"$CXX" -DGS2_CRYPTOPP_ADHOC_MAIN -O2 adhoc.cpp -o "$TMP/adhoc.exe" &>/dev/null
	if [[ ("$?" -eq "0") ]]; then
		OPT=-O2
	fi
fi
...
if [[ -z "$OPT" ]]; then
	rm -f "$TMP/adhoc.exe" &>/dev/null
	"$CXX" -DGS2_CRYPTOPP_ADHOC_MAIN -xO2 adhoc.cpp -o "$TMP/adhoc.exe" &>/dev/null
	if [[ ("$?" -eq "0") ]]; then
		OPT=-xO2
	fi
fi
...
if [[ -z "$OPT" ]]; then
	rm -f "$TMP/adhoc.exe" &>/dev/null
	"$CXX" -DGS2_CRYPTOPP_ADHOC_MAIN -O adhoc.cpp -o "$TMP/adhoc.exe" &>/dev/null
	if [[ ("$?" -eq "0") ]]; then
		OPT=-O
	fi
fi
...
git checkout -f GS2_CRYPTOPP_5_6_2 &>/dev/null
if [[ "$?" -ne "0" ]]; then
	echo "git checkout GS2_CRYPTOPP_5_6_2 failed"
else
...
git checkout -f GS2_CRYPTOPP_5_6_4 &>/dev/null
if [[ "$?" -ne "0" ]]; then
	echo "git checkout GS2_CRYPTOPP_5_6_4 failed"
else
