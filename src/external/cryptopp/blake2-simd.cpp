...
// #undef GS2_CRYPTOPP_SSE41_AVAILABLE
// #undef GS2_CRYPTOPP_ARM_NEON_AVAILABLE
...
#if (defined(__aarch32__) || defined(__aarch64__)) && defined(GS2_CRYPTOPP_SLOW_ARMV8_SHIFT)
# undef GS2_CRYPTOPP_ARM_NEON_AVAILABLE
#endif

#if (GS2_CRYPTOPP_SSE41_AVAILABLE)
# include <emmintrin.h>
# include <tmmintrin.h>
# include <smmintrin.h>
#endif
...
#if GS2_CRYPTOPP_SSE2_INTRIN_AVAILABLE && ((__SUNPRO_CC >= 0x5100 && __SUNPRO_CC < 0x5130) || \
...
#endif  // GS2_CRYPTOPP_SSE41_AVAILABLE
...
void BLAKE2_Compress32_NEON(const byte* input, BLAKE2_State<word32, false>& state)
{
    ...
    GS2_CRYPTOPP_ASSERT(IsAlignedOn(&state.h[0],GetAlignmentOf<uint32x4_t>()));
    GS2_CRYPTOPP_ASSERT(IsAlignedOn(&state.t[0],GetAlignmentOf<uint32x4_t>()));
    GS2_CRYPTOPP_ASSERT(IsAlignedOn(&state.f[0],GetAlignmentOf<uint32x4_t>()));
...
void BLAKE2_Compress64_NEON(const byte* input, BLAKE2_State<word64, true>& state)
{
    ...
    GS2_CRYPTOPP_ASSERT(IsAlignedOn(&state.h[0],GetAlignmentOf<uint64x2_t>()));
    GS2_CRYPTOPP_ASSERT(IsAlignedOn(&state.t[0],GetAlignmentOf<uint64x2_t>()));
    GS2_CRYPTOPP_ASSERT(IsAlignedOn(&state.f[0],GetAlignmentOf<uint64x2_t>()));
