...
double TimerBase::ConvertTo(TimerWord t, Unit unit)
{
	...
	GS2_CRYPTOPP_ASSERT(static_cast<unsigned int>(unit) < COUNTOF(unitsPerSecondTable));
...
unsigned long TimerBase::ElapsedTime()
{
	...
	GS2_CRYPTOPP_ASSERT(elapsed <= (double)ULONG_MAX);
...
TimerWord Timer::GetCurrentTimerValue()
{
	...
	return now.QuadPart;
#elif defined(GS2_CRYPTOPP_UNIX_AVAILABLE)
	timeval now;
...
{
#if defined(GS2_CRYPTOPP_WIN32_AVAILABLE) && defined(THREAD_TIMER_AVAILABLE)
...
#elif defined(GS2_CRYPTOPP_WIN32_AVAILABLE) && !defined(THREAD_TIMER_AVAILABLE)
	LARGE_INTEGER now;
	if (!QueryPerformanceCounter(&now))
	{
		const DWORD lastError = GetLastError();
