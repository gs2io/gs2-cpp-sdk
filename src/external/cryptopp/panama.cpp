...
// use "cl /EP /P /DGS2_CRYPTOPP_GENERATE_X64_MASM panama.cpp" to generate MASM code
...
#if GS2_CRYPTOPP_SSSE3_ASM_AVAILABLE && !defined(GS2_CRYPTOPP_DISABLE_PANAMA_ASM)
	m_state[17] = HasSSSE3();
#endif
}
...
void Panama_SSE2_Pull(size_t count, word32 *state, word32 *z, const word32 *y);
...
#elif GS2_CRYPTOPP_SSE2_ASM_AVAILABLE && !defined(GS2_CRYPTOPP_DISABLE_PANAMA_ASM)
	...
	.endprolog
#else
void GS2_CRYPTOPP_NOINLINE Panama_SSE2_Pull(size_t count, word32 *state, word32 *z, const word32 *y)
{
#if defined(GS2_CRYPTOPP_GNU_STYLE_INLINE_ASSEMBLY)
	asm __volatile__
	(
	INTEL_NOPREFIX
	AS_PUSH_IF86(	bx)
...
#elif GS2_CRYPTOPP_BOOL_X86
	#define REG_loopEnd			[esp]
#elif defined(GS2_CRYPTOPP_GENERATE_X64_MASM)
	...
	ASL(4)
	// gamma and pi
#if GS2_CRYPTOPP_SSSE3_ASM_AVAILABLE
	AS2(	test	AS_REG_6, 1)
	ASJ(	jnz,	6, f)
#endif
...
#endif
#endif	// #ifdef GS2_CRYPTOPP_SSE2_ASM_AVAILABLE
...
{
	GS2_CRYPTOPP_ASSERT(IsAlignedOn(m_state,GetAlignmentOf<word32>()));
...
#define PANAMA_OUTPUT(x)	\
	GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, B::ToEnum(), 0, a(0+9));\
	GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, B::ToEnum(), 1, a(1+9));\
	GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, B::ToEnum(), 2, a(2+9));\
	GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, B::ToEnum(), 3, a(3+9));\
	GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, B::ToEnum(), 4, a(4+9));\
	GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, B::ToEnum(), 5, a(5+9));\
	GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, B::ToEnum(), 6, a(6+9));\
	GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, B::ToEnum(), 7, a(7+9));

			typedef word32 WordType;
			GS2_CRYPTOPP_KEYSTREAM_OUTPUT_SWITCH(PANAMA_OUTPUT, 4*8);
		}
...
{
	GS2_CRYPTOPP_UNUSED(params); GS2_CRYPTOPP_UNUSED(length);
	GS2_CRYPTOPP_ASSERT(length==32);
	memcpy(m_key, key, 32);
}
...
{
	GS2_CRYPTOPP_UNUSED(keystreamBuffer); GS2_CRYPTOPP_UNUSED(iv); GS2_CRYPTOPP_UNUSED(length);
	GS2_CRYPTOPP_ASSERT(IsAlignedOn(iv,GetAlignmentOf<word32>()));
	GS2_CRYPTOPP_ASSERT(length==32);
...
#if (GS2_CRYPTOPP_SSE2_ASM_AVAILABLE || defined(GS2_CRYPTOPP_X64_MASM_AVAILABLE)) && !defined(GS2_CRYPTOPP_DISABLE_PANAMA_ASM)
