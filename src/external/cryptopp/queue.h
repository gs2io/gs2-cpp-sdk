...
#ifndef GS2_CRYPTOPP_QUEUE_H
#define GS2_CRYPTOPP_QUEUE_H
...
class GS2_CRYPTOPP_DLL ByteQueue : public Bufferless<BufferedTransformation>
{
public:
...
class GS2_CRYPTOPP_DLL LazyPutter
{
public:
	LazyPutter(ByteQueue &bq, const byte *inString, size_t size)
		: m_bq(bq) {bq.LazyPut(inString, size);}
	~LazyPutter()
		{try {m_bq.FinalizeLazyPut();} catch(const Exception&) {GS2_CRYPTOPP_ASSERT(0);}}
protected:
...
#ifndef __BORLANDC__
NAMESPACE_BEGIN(std)
template<> inline void swap(gs2::detail::CryptoPP::ByteQueue &a, gs2::detail::CryptoPP::ByteQueue &b)
{
	a.swap(b);
}
NAMESPACE_END
