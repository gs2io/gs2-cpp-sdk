...
#ifndef GS2_CRYPTOPP_BLAKE2_H
#define GS2_CRYPTOPP_BLAKE2_H
...
struct BLAKE2_Info : public VariableKeyLength<(T_64bit ? 64 : 32),0,(T_64bit ? 64 : 32),1,SimpleKeyingInterface::NOT_RESYNCHRONIZABLE>
{
	...
	GS2_CRYPTOPP_CONSTANT(MIN_KEYLENGTH = KeyBase::MIN_KEYLENGTH)
	GS2_CRYPTOPP_CONSTANT(MAX_KEYLENGTH = KeyBase::MAX_KEYLENGTH)
	GS2_CRYPTOPP_CONSTANT(DEFAULT_KEYLENGTH = KeyBase::DEFAULT_KEYLENGTH)

	GS2_CRYPTOPP_CONSTANT(BLOCKSIZE = (T_64bit ? 128 : 64))
	GS2_CRYPTOPP_CONSTANT(DIGESTSIZE = (T_64bit ? 64 : 32))
	GS2_CRYPTOPP_CONSTANT(SALTSIZE = (T_64bit ? 16 : 8))
	GS2_CRYPTOPP_CONSTANT(PERSONALIZATIONSIZE = (T_64bit ? 16 : 8))

	GS2_CRYPTOPP_STATIC_CONSTEXPR const char* StaticAlgorithmName() {return (T_64bit ? "BLAKE2b" : "BLAKE2s");}
};
...
/// \since Crypto++ 5.6.4
template <bool T_64bit>
struct GS2_CRYPTOPP_NO_VTABLE BLAKE2_ParameterBlock
{
};

/// \brief BLAKE2b parameter block specialization
template<>
struct GS2_CRYPTOPP_NO_VTABLE BLAKE2_ParameterBlock<true>
{
	GS2_CRYPTOPP_CONSTANT(SALTSIZE = BLAKE2_Info<true>::SALTSIZE)
	GS2_CRYPTOPP_CONSTANT(DIGESTSIZE = BLAKE2_Info<true>::DIGESTSIZE)
	GS2_CRYPTOPP_CONSTANT(PERSONALIZATIONSIZE = BLAKE2_Info<true>::PERSONALIZATIONSIZE)
	...
	{
		GS2_CRYPTOPP_ASSERT(digestSize <= DIGESTSIZE);
		memset(this, 0x00, sizeof(*this));
...
template<>
struct GS2_CRYPTOPP_NO_VTABLE BLAKE2_ParameterBlock<false>
{
	GS2_CRYPTOPP_CONSTANT(SALTSIZE = BLAKE2_Info<false>::SALTSIZE)
	GS2_CRYPTOPP_CONSTANT(DIGESTSIZE = BLAKE2_Info<false>::DIGESTSIZE)
	GS2_CRYPTOPP_CONSTANT(PERSONALIZATIONSIZE = BLAKE2_Info<false>::PERSONALIZATIONSIZE)
...
struct GS2_CRYPTOPP_NO_VTABLE BLAKE2_State
{
	GS2_CRYPTOPP_CONSTANT(BLOCKSIZE = BLAKE2_Info<T_64bit>::BLOCKSIZE)
...
{
public:
	GS2_CRYPTOPP_CONSTANT(DEFAULT_KEYLENGTH = BLAKE2_Info<T_64bit>::DEFAULT_KEYLENGTH)
	GS2_CRYPTOPP_CONSTANT(MIN_KEYLENGTH = BLAKE2_Info<T_64bit>::MIN_KEYLENGTH)
	GS2_CRYPTOPP_CONSTANT(MAX_KEYLENGTH = BLAKE2_Info<T_64bit>::MAX_KEYLENGTH)

	GS2_CRYPTOPP_CONSTANT(DIGESTSIZE = BLAKE2_Info<T_64bit>::DIGESTSIZE)
	GS2_CRYPTOPP_CONSTANT(BLOCKSIZE = BLAKE2_Info<T_64bit>::BLOCKSIZE)
	GS2_CRYPTOPP_CONSTANT(SALTSIZE = BLAKE2_Info<T_64bit>::SALTSIZE)
	GS2_CRYPTOPP_CONSTANT(PERSONALIZATIONSIZE = BLAKE2_Info<T_64bit>::PERSONALIZATIONSIZE)
	...
	GS2_CRYPTOPP_STATIC_CONSTEXPR const char* StaticAlgorithmName() {return BLAKE2_Info<T_64bit>::StaticAlgorithmName();}
	...
	unsigned int OptimalDataAlignment() const {return (GS2_CRYPTOPP_BOOL_ALIGN16 ? 16 : GetAlignmentOf<W>());}
...
protected:
	...
	void UncheckedSetKey(const byte* key, unsigned int length, const gs2::detail::CryptoPP::NameValuePairs& params);
...
public:
	...
	GS2_CRYPTOPP_COMPILE_ASSERT(sizeof(ParameterBlock) == 64);
...
public:
	...
	GS2_CRYPTOPP_COMPILE_ASSERT(sizeof(ParameterBlock) == 32);
