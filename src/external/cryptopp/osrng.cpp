...
#if defined(GS2_CRYPTOPP_WIN32_AVAILABLE) && !defined(OS_RNG_AVAILABLE)
...
# define GS2_CRYPTOPP_CONTAINER L"Crypto++ RNG"
#else
# define GS2_CRYPTOPP_CONTAINER "Crypto++ RNG"
#endif
...
MicrosoftCryptoProvider::MicrosoftCryptoProvider() : m_hProvider(0)
{
	...
	if (!CryptAcquireContext(&m_hProvider, 0, 0, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT))
	{
		...
		if (!CryptAcquireContext(&m_hProvider, GS2_CRYPTOPP_CONTAINER, 0, PROV_RSA_FULL, CRYPT_NEWKEYSET /*user*/) &&
		    !CryptAcquireContext(&m_hProvider, GS2_CRYPTOPP_CONTAINER, 0, PROV_RSA_FULL, CRYPT_MACHINE_KEYSET|CRYPT_NEWKEYSET))
		{
...
{
#ifndef GS2_CRYPTOPP_WIN32_AVAILABLE
...
#ifndef GS2_CRYPTOPP_BLOCKING_RNG_FILENAME
#ifdef __OpenBSD__
#define GS2_CRYPTOPP_BLOCKING_RNG_FILENAME "/dev/srandom"
#else
#define GS2_CRYPTOPP_BLOCKING_RNG_FILENAME "/dev/random"
#endif
#endif

BlockingRng::BlockingRng()
{
	m_fd = open(GS2_CRYPTOPP_BLOCKING_RNG_FILENAME,O_RDONLY);
	if (m_fd == -1)
		throw OS_RNG_Err("open " GS2_CRYPTOPP_BLOCKING_RNG_FILENAME);
}
...
void BlockingRng::GenerateBlock(byte *output, size_t size)
{
	while (size)
	{
		...
		if (len < 0)
		{
			...
			if (errno != EINTR && errno != EAGAIN)
				throw OS_RNG_Err("read " GS2_CRYPTOPP_BLOCKING_RNG_FILENAME);
