...
void TestKeyDerivationFunction(TestData &v)
{
	...
	if(test == "Skip") return;
	GS2_CRYPTOPP_ASSERT(test == "Verify");
...
void OutputPair(const NameValuePairs &v, const char *name)
{
	...
	GS2_CRYPTOPP_UNUSED(b); GS2_CRYPTOPP_ASSERT(b);
	std::cout << name << ": \\\n    ";
...
{
	static const std::string dataDirectory(GS2_CRYPTOPP_DATA_DIR);
	...
	while (file)
	{
		...
		// GS2_CRYPTOPP_ASSERT(!value.empty());
		v[name] = value;
		...
		if (name == "Test" && (s_thorough || v["SlowTest"] != "1"))
		{
			...
			catch (const gs2::detail::CryptoPP::Exception &e)
			{
				std::cout << "\ngs2::detail::CryptoPP::Exception caught: " << e.what() << std::endl;
			}
