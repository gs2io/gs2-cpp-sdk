...
#ifndef GS2_CRYPTOPP_POLY1305_H
#define GS2_CRYPTOPP_POLY1305_H
...
/// \since Crypto++ 6.0
template <class T>
class GS2_CRYPTOPP_NO_VTABLE Poly1305_Base : public FixedKeyLength<32, SimpleKeyingInterface::UNIQUE_IV, 16>, public MessageAuthenticationCode
{
	GS2_CRYPTOPP_COMPILE_ASSERT(T::DEFAULT_KEYLENGTH == 16);
	GS2_CRYPTOPP_COMPILE_ASSERT(T::BLOCKSIZE == 16);

public:
	static std::string StaticAlgorithmName() {return std::string("Poly1305(") + T::StaticAlgorithmName() + ")";}

	GS2_CRYPTOPP_CONSTANT(DIGESTSIZE=T::BLOCKSIZE)
	GS2_CRYPTOPP_CONSTANT(BLOCKSIZE=T::BLOCKSIZE)
...
{
public:
	GS2_CRYPTOPP_CONSTANT(DEFAULT_KEYLENGTH=Poly1305_Base<T>::DEFAULT_KEYLENGTH)
