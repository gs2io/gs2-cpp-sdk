...
{
	GS2_CRYPTOPP_ASSERT(m_s == INVALID_SOCKET);
...
# if defined(USE_WINDOWS8_API)
		...
		GS2_CRYPTOPP_ASSERT(result || (!result && GetLastError() == ERROR_NOT_FOUND));
		CheckAndHandleError_int("closesocket", closesocket(m_s));
		GS2_CRYPTOPP_UNUSED(result);	// Used by GS2_CRYPTOPP_ASSERT in debug builds
# else
		BOOL result = CancelIo((HANDLE) m_s);
		GS2_CRYPTOPP_ASSERT(result || (!result && GetLastError() == ERROR_NOT_FOUND));
		CheckAndHandleError_int("closesocket", closesocket(m_s));
		GS2_CRYPTOPP_UNUSED(result);
# endif
#else
...
{
	GS2_CRYPTOPP_ASSERT(addr != NULLPTR);
...
{
	GS2_CRYPTOPP_ASSERT(!m_resultPending && !m_eofReceived);
...
{
	GS2_CRYPTOPP_ASSERT(!m_resultPending);
	DWORD written = 0;
	// don't queue too much at once, or we might use up non-paged memory
	WSABUF wsabuf = {UnsignedMin((u_long)128*1024, bufLen), (char *)buf};
