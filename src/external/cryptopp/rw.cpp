...
#if defined(_OPENMP)
static const bool GS2_CRYPTOPP_RW_USE_OMP = true;
#else
static const bool GS2_CRYPTOPP_RW_USE_OMP = false;
#endif
...
void InvertibleRWFunction::PrecomputeTweakedRoots() const
{
	...
	#pragma omp parallel sections if(GS2_CRYPTOPP_RW_USE_OMP)
	{
		#pragma omp section
...
Integer InvertibleRWFunction::CalculateInverse(RandomNumberGenerator &rng, const Integer &x) const
{
	...
	GS2_CRYPTOPP_ASSERT((e * f * s.Squared()) % m_n == x);
