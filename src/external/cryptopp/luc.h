...
#ifndef GS2_CRYPTOPP_LUC_H
#define GS2_CRYPTOPP_LUC_H
...
public:
	...
	const AbstractGroup<Element> & GetGroup() const {GS2_CRYPTOPP_ASSERT(false); throw 0;}
...
public:
	...
	void SetBase(const DL_GroupPrecomputation<Element> &group, const Integer &base)
		{GS2_CRYPTOPP_UNUSED(group); m_g = base;}
	const Integer & GetBase(const DL_GroupPrecomputation<Element> &group) const
		{GS2_CRYPTOPP_UNUSED(group); return m_g;}
	void Precompute(const DL_GroupPrecomputation<Element> &group, unsigned int maxExpBits, unsigned int storage)
		{GS2_CRYPTOPP_UNUSED(group); GS2_CRYPTOPP_UNUSED(maxExpBits); GS2_CRYPTOPP_UNUSED(storage);}
	void Load(const DL_GroupPrecomputation<Element> &group, BufferedTransformation &storedPrecomputation)
		{GS2_CRYPTOPP_UNUSED(group); GS2_CRYPTOPP_UNUSED(storedPrecomputation);}
	...
	Integer CascadeExponentiate(const DL_GroupPrecomputation<Element> &group, const Integer &exponent, const DL_FixedBasePrecomputation<Integer> &pc2, const Integer &exponent2) const
		{
			GS2_CRYPTOPP_UNUSED(group); GS2_CRYPTOPP_UNUSED(exponent); GS2_CRYPTOPP_UNUSED(pc2); GS2_CRYPTOPP_UNUSED(exponent2);
			// shouldn't be called
...
public:
	...
	{
		GS2_CRYPTOPP_UNUSED(a); GS2_CRYPTOPP_UNUSED(b);
		throw NotImplemented("LUC_GroupParameters: MultiplyElements can not be implemented");
	}
	Element CascadeExponentiate(const Element &element1, const Integer &exponent1, const Element &element2, const Integer &exponent2) const
	{
		GS2_CRYPTOPP_UNUSED(element1); GS2_CRYPTOPP_UNUSED(exponent1); GS2_CRYPTOPP_UNUSED(element2); GS2_CRYPTOPP_UNUSED(exponent2);
		throw NotImplemented("LUC_GroupParameters: MultiplyElements can not be implemented");
	}
...
{
public:
	GS2_CRYPTOPP_STATIC_CONSTEXPR const char* StaticAlgorithmName() {return "LUC-HMP";}
...
struct LUC_IES
	: public DL_ES<
		...
		LUC_IES<> >
{
	GS2_CRYPTOPP_STATIC_CONSTEXPR const char* StaticAlgorithmName() {return "LUC-IES";}	// non-standard name
};
