...
GS2_CRYPTOPP_COMPILE_ASSERT(sizeof(byte) == 1);
GS2_CRYPTOPP_COMPILE_ASSERT(sizeof(word16) == 2);
GS2_CRYPTOPP_COMPILE_ASSERT(sizeof(word32) == 4);
GS2_CRYPTOPP_COMPILE_ASSERT(sizeof(word64) == 8);
#ifdef GS2_CRYPTOPP_NATIVE_DWORD_AVAILABLE
GS2_CRYPTOPP_COMPILE_ASSERT(sizeof(dword) == 2*sizeof(word));
#endif
...
{
	GS2_CRYPTOPP_ASSERT(MinLastBlockSize() == 0);	// this function should be overridden otherwise
...
size_t StreamTransformation::ProcessLastBlock(byte *outString, size_t outLength, const byte *inString, size_t inLength)
{
	...
	GS2_CRYPTOPP_ASSERT(MinLastBlockSize() == 0);
...
// be instantiable. Its now spilt milk, and we are going to GS2_CRYPTOPP_ASSERT it in Debug
...
void BufferedTransformation::Initialize(const NameValuePairs &parameters, int propagation)
{
	...
	GS2_CRYPTOPP_ASSERT(!AttachedTransformation());
...
bool BufferedTransformation::GetNextMessage()
{
	...
	{
		GS2_CRYPTOPP_ASSERT(!AnyMessages());
	}
	return result;
}
...
size_t BufferedTransformation::TransferMessagesTo2(BufferedTransformation &target, unsigned int &messageCount, const std::string &channel, bool blocking)
{
	...
	else
	{
		...
		for (messageCount=0; messageCount < maxMessages && AnyMessages(); messageCount++)
		{
			...
			GS2_CRYPTOPP_UNUSED(result); GS2_CRYPTOPP_ASSERT(result);
		}
		return 0;
	}
}
...
size_t BufferedTransformation::TransferAllTo2(BufferedTransformation &target, const std::string &channel, bool blocking)
{
	...
	{
		GS2_CRYPTOPP_ASSERT(!NumberOfMessageSeries());
...
#ifndef GS2_CRYPTOPP_BUILD_VERSION
# define GS2_CRYPTOPP_BUILD_VERSION GS2_CRYPTOPP_VERSION
#endif
int LibraryVersion(GS2_CRYPTOPP_NOINLINE_DOTDOTDOT)
{
	return GS2_CRYPTOPP_BUILD_VERSION;
}
