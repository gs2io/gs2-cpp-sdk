...
#if GS2_CRYPTOPP_FAST_ROTATE(32)
	b = rotrConstant<30>(b);
...
GF2_32::Element GF2_32::MultiplicativeInverse(Element a) const
{
	...
	while (g0 != 1)
	{
		...
		{
			GS2_CRYPTOPP_ASSERT(BitPrecision(g1) <= BitPrecision(g0));
			g2 = g1;
			v2 = v1;
		}
		else
		...
		while ((g0^g2) >= g2)
		{
			GS2_CRYPTOPP_ASSERT(BitPrecision(g0) > BitPrecision(g2));
			g2 <<= 1;
			v2 <<= 1;
		}
