...
#ifndef GS2_CRYPTOPP_NIST_DRBG_H
#define GS2_CRYPTOPP_NIST_DRBG_H
...
{
public:
    GS2_CRYPTOPP_CONSTANT(SECURITY_STRENGTH=STRENGTH)
    GS2_CRYPTOPP_CONSTANT(SEED_LENGTH=SEEDLENGTH)
    GS2_CRYPTOPP_CONSTANT(MINIMUM_ENTROPY=STRENGTH)
    GS2_CRYPTOPP_CONSTANT(MINIMUM_NONCE=0)
    GS2_CRYPTOPP_CONSTANT(MINIMUM_ADDITIONAL=0)
    GS2_CRYPTOPP_CONSTANT(MINIMUM_PERSONALIZATION=0)
    GS2_CRYPTOPP_CONSTANT(MAXIMUM_ENTROPY=INT_MAX)
    GS2_CRYPTOPP_CONSTANT(MAXIMUM_NONCE=INT_MAX)
    GS2_CRYPTOPP_CONSTANT(MAXIMUM_ADDITIONAL=INT_MAX)
    GS2_CRYPTOPP_CONSTANT(MAXIMUM_PERSONALIZATION=INT_MAX)
    GS2_CRYPTOPP_CONSTANT(MAXIMUM_BYTES_PER_REQUEST=65536)
    GS2_CRYPTOPP_CONSTANT(MAXIMUM_REQUESTS_BEFORE_RESEED=INT_MAX)
    ...
    const byte* personalization, size_t personalizationLength)
{
    ...
    GS2_CRYPTOPP_ASSERT(entropyLength >= MINIMUM_ENTROPY);
    ...
    GS2_CRYPTOPP_ASSERT(entropyLength <= MAXIMUM_ENTROPY);
    GS2_CRYPTOPP_ASSERT(nonceLength <= MAXIMUM_NONCE);
    GS2_CRYPTOPP_ASSERT(personalizationLength <= MAXIMUM_PERSONALIZATION);
...
void Hash_DRBG<HASH, STRENGTH, SEEDLENGTH>::DRBG_Reseed(const byte* entropy, size_t entropyLength, const byte* additional, size_t additionaLength)
{
    ...
    // or too large a persoanlization string. We warn in Debug builds, but do nothing in Release builds.
    GS2_CRYPTOPP_ASSERT(entropyLength <= MAXIMUM_ENTROPY);
    GS2_CRYPTOPP_ASSERT(additionaLength <= MAXIMUM_ADDITIONAL);
...
void Hash_DRBG<HASH, STRENGTH, SEEDLENGTH>::Hash_Generate(const byte* additional, size_t additionaLength, byte *output, size_t size)
{
    ...
    if (additional && additionaLength)
    {
        ...
        GS2_CRYPTOPP_ASSERT(SEEDLENGTH >= HASH::DIGESTSIZE);
    ...
    // Steps 4-7
    {
        ...
        GS2_CRYPTOPP_ASSERT(SEEDLENGTH >= HASH::DIGESTSIZE);
        GS2_CRYPTOPP_ASSERT(HASH::DIGESTSIZE >= sizeof(m_reseed));
...
#elif defined(GS2_CRYPTOPP_MSC_VERSION)
        byte* p1 = m_v.begin()+SEEDLENGTH-8;
        byte* p2 = m_c.begin()+SEEDLENGTH-8;
        byte* p3 = h.begin()+HASH::DIGESTSIZE-8;
        ...
        // GS2_CRYPTOPP_WORD128_AVAILABLE causes -Wunused-but-set-variable
        GS2_CRYPTOPP_UNUSED(k);
    }
...
#endif  // GS2_CRYPTOPP_NIST_DRBG_H
