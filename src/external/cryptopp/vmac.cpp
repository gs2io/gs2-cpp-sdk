...
#if defined(GS2_CRYPTOPP_DISABLE_VMAC_ASM)
...
#if defined(_MSC_VER) && !GS2_CRYPTOPP_BOOL_SLOW_WORD64
#include <intrin.h>
#endif

#if defined(GS2_CRYPTOPP_WORD128_AVAILABLE) && !defined(GS2_CRYPTOPP_X64_ASM_AVAILABLE)
...
void VMAC_Base::UncheckedSetKey(const byte *userKey, unsigned int keylength, const NameValuePairs &params)
{
	...
	GS2_CRYPTOPP_ASSERT(IsAlignedOn(l3Key,GetAlignmentOf<word64>()));
...
{
	GS2_CRYPTOPP_ASSERT(IsAlignedOn(m_polyState(),GetAlignmentOf<word64>()));
	GS2_CRYPTOPP_ASSERT(IsAlignedOn(m_nhKey(),GetAlignmentOf<word64>()));
	...
	GS2_CRYPTOPP_UNUSED(data); GS2_CRYPTOPP_UNUSED(tagPart); GS2_CRYPTOPP_UNUSED(L1KeyLength);
	GS2_CRYPTOPP_UNUSED(blocksRemainingInWord64);
...
#undef k3
	...
	#endif
	#if defined(GS2_CRYPTOPP_X64_ASM_AVAILABLE)
	...
	#elif defined(_MSC_VER) && !GS2_CRYPTOPP_BOOL_SLOW_WORD64
		#define DeclareNH(a) word64 a##0=0, a##1=0
...
{
	GS2_CRYPTOPP_ASSERT(IsAlignedOn(DataBuf(),GetAlignmentOf<word64>()));
	GS2_CRYPTOPP_ASSERT(IsAlignedOn(m_polyState(),GetAlignmentOf<word64>()));
