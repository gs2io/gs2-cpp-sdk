...
// for memalign
#if defined(GS2_CRYPTOPP_MEMALIGN_AVAILABLE) || defined(GS2_CRYPTOPP_MM_MALLOC_AVAILABLE) || defined(QNX)
# include <malloc.h>
#endif
// for posix_memalign
#if defined(GS2_CRYPTOPP_POSIX_MEMALIGN_AVAILABLE)
# include <stdlib.h>
#endif
...
{
	GS2_CRYPTOPP_ASSERT(buf != NULLPTR);
	GS2_CRYPTOPP_ASSERT(mask != NULLPTR);
	GS2_CRYPTOPP_ASSERT(count > 0);

	size_t i=0;
	if (IsAligned<word32>(buf) && IsAligned<word32>(mask))
	{
		if (!GS2_CRYPTOPP_BOOL_SLOW_WORD64 && IsAligned<word64>(buf) && IsAligned<word64>(mask))
		{
			for (i=0; i<count/8; i++)
...
{
	GS2_CRYPTOPP_ASSERT(output != NULLPTR);
	GS2_CRYPTOPP_ASSERT(input != NULLPTR);
	GS2_CRYPTOPP_ASSERT(count > 0);

	size_t i=0;
	if (IsAligned<word32>(output) && IsAligned<word32>(input) && IsAligned<word32>(mask))
	{
		if (!GS2_CRYPTOPP_BOOL_SLOW_WORD64 && IsAligned<word64>(output) && IsAligned<word64>(input) && IsAligned<word64>(mask))
		{
			for (i=0; i<count/8; i++)
...
std::string StringNarrow(const wchar_t *str, bool throwOnError)
{
	...
	GS2_CRYPTOPP_ASSERT(size != (size_t)-1);
	if (size == (size_t)-1)
	{
		if (throwOnError)
...
{
	byte *p;
#if defined(GS2_CRYPTOPP_MM_MALLOC_AVAILABLE)
	while ((p = (byte *)_mm_malloc(size, 16)) == NULLPTR)
#elif defined(GS2_CRYPTOPP_MEMALIGN_AVAILABLE)
	while ((p = (byte *)memalign(16, size)) == NULLPTR)
#elif defined(GS2_CRYPTOPP_MALLOC_ALIGNMENT_IS_16)
	while ((p = (byte *)malloc(size)) == NULLPTR)
#elif defined(GS2_CRYPTOPP_POSIX_MEMALIGN_AVAILABLE)
...
#ifdef GS2_CRYPTOPP_NO_ALIGNED_ALLOC
	size_t adjustment = 16-((size_t)p%16);
	p += adjustment;
	p[-1] = (byte)adjustment;
#endif

	GS2_CRYPTOPP_ASSERT(IsAlignedOn(p, 16));
	return p;
}

void AlignedDeallocate(void *p)
{
#ifdef GS2_CRYPTOPP_MM_MALLOC_AVAILABLE
	_mm_free(p);
#elif defined(GS2_CRYPTOPP_NO_ALIGNED_ALLOC)
	p = (byte *)p - ((byte *)p)[-1];
	free(p);
#else
	free(p);
