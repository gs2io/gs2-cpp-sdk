...
bool EC2N::DecodePoint(EC2N::Point &P, BufferedTransformation &bt, size_t encodedPointLen) const
{
	...
	case 3:
	{
		...
		GS2_CRYPTOPP_ASSERT(P.x == m_field->SquareRoot(z));
		P.y = m_field->Divide(m_field->Add(m_field->Multiply(z, m_field->Add(P.x, m_a)), m_b), z);
		GS2_CRYPTOPP_ASSERT(P.x == m_field->Subtract(m_field->Divide(m_field->Subtract(m_field->Multiply(P.y, z), m_b), z), m_a));
		z = m_field->SolveQuadraticEquation(P.y);
		GS2_CRYPTOPP_ASSERT(m_field->Add(m_field->Square(z), z) == P.y);
...
void EC2N::EncodePoint(byte *encodedPoint, const Point &P, bool compressed) const
{
	...
	GS2_CRYPTOPP_ASSERT(sink.TotalPutLength() == EncodedPointSize(compressed));
}
