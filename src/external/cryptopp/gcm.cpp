...
// use "cl /EP /P /DGS2_CRYPTOPP_GENERATE_X64_MASM gcm.cpp" to generate MASM code
...
# undef GS2_CRYPTOPP_CLMUL_AVAILABLE
#endif

#if (GS2_CRYPTOPP_SSE2_INTRIN_AVAILABLE)
# include <emmintrin.h>
#endif
...
#if (GS2_CRYPTOPP_LLVM_CLANG_VERSION >= 30600) || (GS2_CRYPTOPP_APPLE_CLANG_VERSION >= 70000) || defined(GS2_CRYPTOPP_CLANG_INTEGRATED_ASSEMBLER)
...
#if GS2_CRYPTOPP_ARM_NEON_AVAILABLE
...
{
    GS2_CRYPTOPP_ASSERT(IsAlignedOn(a,GetAlignmentOf<word64>()));
    GS2_CRYPTOPP_ASSERT(IsAlignedOn(b,GetAlignmentOf<word64>()));
    GS2_CRYPTOPP_ASSERT(IsAlignedOn(c,GetAlignmentOf<word64>()));
    ((word64 *)(void *)a)[0] = ((word64 *)(void *)b)[0] ^ ((word64 *)(void *)c)[0];
    ((word64 *)(void *)a)[1] = ((word64 *)(void *)b)[1] ^ ((word64 *)(void *)c)[1];
}

#if GS2_CRYPTOPP_SSE2_INTRIN_AVAILABLE || GS2_CRYPTOPP_SSE2_ASM_AVAILABLE
...
#if GS2_CRYPTOPP_CLMUL_AVAILABLE
...
#endif  // GS2_CRYPTOPP_CLMUL_AVAILABLE

#if GS2_CRYPTOPP_ARM_PMULL_AVAILABLE
...
#endif  // GS2_CRYPTOPP_ARM_PMULL_AVAILABLE
...
void GCM_Base::SetKeyWithoutResync(const byte *userKey, size_t keylength, const NameValuePairs &params)
{
    ...
    GS2_CRYPTOPP_ASSERT(blockSize == REQUIRED_BLOCKSIZE);
    ...
    if (HasCLMUL())
    {
        ...
        GS2_CRYPTOPP_ASSERT(tableSize > static_cast<int>(blockSize));
    }
    else
#elif GS2_CRYPTOPP_ARM_PMULL_AVAILABLE
    if (HasPMULL())
    {
        // Avoid "parameter not used" error and suppress Coverity finding
        (void)params.GetIntValue(Name::TableSize(), tableSize);
...
#elif GS2_CRYPTOPP_ARM_NEON_AVAILABLE
            if (HasNEON())
                for (j=2; j<=0x80; j*=2)
                    for (k=1; k<j; k++)
...
{
    return
#if GS2_CRYPTOPP_SSE2_ASM_AVAILABLE || defined(GS2_CRYPTOPP_X64_MASM_AVAILABLE)
        HasSSE2() ? 16 :
#elif GS2_CRYPTOPP_ARM_NEON_AVAILABLE
        HasNEON() ? 4 :
#endif
...
size_t GCM_Base::AuthenticateBlocks(const byte *data, size_t len)
{
    ...
    GS2_CRYPTOPP_ASSERT(IsAlignedOn(hashBuffer,GetAlignmentOf<word64>()));
