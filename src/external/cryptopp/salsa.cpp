...
// use "cl /EP /P /DGS2_CRYPTOPP_GENERATE_X64_MASM salsa.cpp" to generate MASM code
...
#if defined(GS2_CRYPTOPP_DISABLE_SALSA_ASM)
...
{
	GS2_CRYPTOPP_ASSERT(data != NULLPTR);
	GS2_CRYPTOPP_ASSERT(rounds % 2 == 0);

	GS2_CRYPTOPP_ALIGN_DATA(16) word32 x[16];
...
{
	GS2_CRYPTOPP_UNUSED(keystreamBuffer), GS2_CRYPTOPP_UNUSED(length);
	GS2_CRYPTOPP_ASSERT(length==8);
...
{
#if GS2_CRYPTOPP_SSE2_ASM_AVAILABLE
	if (HasSSE2())
		return 16;
	else
#endif
...
{
#endif	// #ifdef GS2_CRYPTOPP_GENERATE_X64_MASM
	...
	if (HasSSE2())
	{
		...
		GS2_CRYPTOPP_ALIGN_DATA(16) byte workspace[16*32];
	#else
...
#ifndef GS2_CRYPTOPP_GENERATE_X64_MASM
	{
		...
		while (iterationCount--)
		{
			...
			#define SALSA_OUTPUT(x)	{\
				GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 0, x0 + m_state[0]);\
				GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 1, x13 + m_state[13]);\
				GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 2, x10 + m_state[10]);\
				GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 3, x7 + m_state[7]);\
				GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 4, x4 + m_state[4]);\
				GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 5, x1 + m_state[1]);\
				GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 6, x14 + m_state[14]);\
				GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 7, x11 + m_state[11]);\
				GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 8, x8 + m_state[8]);\
				GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 9, x5 + m_state[5]);\
				GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 10, x2 + m_state[2]);\
				GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 11, x15 + m_state[15]);\
				GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 12, x12 + m_state[12]);\
				GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 13, x9 + m_state[9]);\
				GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 14, x6 + m_state[6]);\
				GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, LITTLE_ENDIAN_ORDER, 15, x3 + m_state[3]);}

#ifndef GS2_CRYPTOPP_DOXYGEN_PROCESSING
			GS2_CRYPTOPP_KEYSTREAM_OUTPUT_SWITCH(SALSA_OUTPUT, BYTES_PER_ITERATION);
#endif
