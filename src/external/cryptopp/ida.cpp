...
const gs2::detail::CryptoPP::GF2_32 field;
NAMESPACE_END
NAMESPACE_END
NAMESPACE_END
...
void RawIDA::IsolatedInitialize(const NameValuePairs &parameters)
{
	...
	GS2_CRYPTOPP_ASSERT(m_threshold > 0);
	if (m_threshold <= 0)
...
{
	GS2_CRYPTOPP_ASSERT(m_inputChannelIds.size() == size_t(m_threshold));
