...
# undef GS2_CRYPTOPP_ARM_PMULL_AVAILABLE
#endif
...
// Can't use GS2_CRYPTOPP_ARM_XXX_AVAILABLE because too many
// compilers don't follow ACLE conventions for the include.
#if defined(GS2_CRYPTOPP_ARM_ACLE_AVAILABLE)
# include <stdint.h>
# include <arm_acle.h>
#endif
...
# if (GS2_CRYPTOPP_GCC_VERSION >= 40800) && (GS2_CRYPTOPP_GCC_VERSION < 49000)
...
bool CPU_ProbePMULL()
{
#if defined(GS2_CRYPTOPP_NO_CPU_FEATURE_PROBES)
	return false;
#elif (GS2_CRYPTOPP_ARM_PMULL_AVAILABLE)
# if defined(GS2_CRYPTOPP_MS_STYLE_INLINE_ASSEMBLY)
...
#else
    return false;
#endif  // GS2_CRYPTOPP_ARM_SHA_AVAILABLE
}
...
{
    GS2_CRYPTOPP_ASSERT(IsAlignedOn(a,GetAlignmentOf<uint64x2_t>()));
    GS2_CRYPTOPP_ASSERT(IsAlignedOn(b,GetAlignmentOf<uint64x2_t>()));
    GS2_CRYPTOPP_ASSERT(IsAlignedOn(c,GetAlignmentOf<uint64x2_t>()));
...
{
# if GS2_CRYPTOPP_SSE2_ASM_AVAILABLE && defined(__GNUC__)
    asm ("movdqa %1, %%xmm0; pxor %2, %%xmm0; movdqa %%xmm0, %0;"
         : "=m" (a[0]) : "m"(b[0]), "m"(c[0]));
# else  // GS2_CRYPTOPP_SSE2_INTRIN_AVAILABLE
