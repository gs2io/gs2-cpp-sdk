...
#ifndef GS2_CRYPTOPP_STRCIPHR_H
#define GS2_CRYPTOPP_STRCIPHR_H
...
class GS2_CRYPTOPP_NO_VTABLE AbstractPolicyHolder : public BASE
{
public:
...
struct GS2_CRYPTOPP_DLL GS2_CRYPTOPP_NO_VTABLE AdditiveCipherAbstractPolicy
{
	...
	virtual void OperateKeystream(KeystreamOperation operation, byte *output, const byte *input, size_t iterationCount)
		{GS2_CRYPTOPP_UNUSED(operation); GS2_CRYPTOPP_UNUSED(output); GS2_CRYPTOPP_UNUSED(input); GS2_CRYPTOPP_UNUSED(iterationCount); GS2_CRYPTOPP_ASSERT(false);}
	...
	virtual void CipherResynchronize(byte *keystreamBuffer, const byte *iv, size_t length)
		{GS2_CRYPTOPP_UNUSED(keystreamBuffer); GS2_CRYPTOPP_UNUSED(iv); GS2_CRYPTOPP_UNUSED(length); throw NotImplemented("SimpleKeyingInterface: this object doesn't support resynchronization");}
	...
	virtual void SeekToIteration(lword iterationCount)
		{GS2_CRYPTOPP_UNUSED(iterationCount); GS2_CRYPTOPP_ASSERT(!CipherIsRandomAccess()); throw NotImplemented("StreamTransformation: this object doesn't support random access");}
};
...
struct GS2_CRYPTOPP_NO_VTABLE AdditiveCipherConcretePolicy : public BASE
{
	typedef WT WordType;
	GS2_CRYPTOPP_CONSTANT(BYTES_PER_ITERATION = sizeof(WordType) * W)

#if !(GS2_CRYPTOPP_BOOL_X86 || GS2_CRYPTOPP_BOOL_X64)
	/// \brief Provides data alignment requirements
	/// \returns data alignment requirements, in bytes
	/// \details Internally, the default implementation returns 1. If the stream cipher is implemented
	///   using an SSE2 ASM or intrinsics, then the value returned is usually 16.
...
#define GS2_CRYPTOPP_KEYSTREAM_OUTPUT_WORD(x, b, i, a)	\
...
/// \brief Helper macro to implement OperateKeystream
#define GS2_CRYPTOPP_KEYSTREAM_OUTPUT_XMM(x, i, a)	{\
...
/// \brief Helper macro to implement OperateKeystream
#define GS2_CRYPTOPP_KEYSTREAM_OUTPUT_SWITCH(x, y)	\
	switch (operation)							\
	{											\
...
class GS2_CRYPTOPP_NO_VTABLE AdditiveCipherTemplate : public BASE, public RandomNumberGenerator
{
public:
...
class GS2_CRYPTOPP_DLL GS2_CRYPTOPP_NO_VTABLE CFB_CipherAbstractPolicy
{
public:
	...
	virtual void Iterate(byte *output, const byte *input, CipherDir dir, size_t iterationCount)
		{GS2_CRYPTOPP_UNUSED(output); GS2_CRYPTOPP_UNUSED(input); GS2_CRYPTOPP_UNUSED(dir); GS2_CRYPTOPP_UNUSED(iterationCount);
		 GS2_CRYPTOPP_ASSERT(false); /*throw 0;*/ throw Exception(Exception::OTHER_ERROR, "SimpleKeyingInterface: unexpected error");}
	...
	virtual void CipherResynchronize(const byte *iv, size_t length)
		{GS2_CRYPTOPP_UNUSED(iv); GS2_CRYPTOPP_UNUSED(length); throw NotImplemented("SimpleKeyingInterface: this object doesn't support resynchronization");}
};
...
struct GS2_CRYPTOPP_NO_VTABLE CFB_CipherConcretePolicy : public BASE
{
	typedef WT WordType;
	...
	struct RegisterOutput
	{
		...
		{
			GS2_CRYPTOPP_ASSERT(IsAligned<WordType>(m_output));
			GS2_CRYPTOPP_ASSERT(IsAligned<WordType>(m_input));
			...
			if (m_dir == ENCRYPTION)
			{
				if (m_input == NULLPTR)
				{
					GS2_CRYPTOPP_ASSERT(m_output == NULLPTR);
				}
				else
				{
...
/// \tparam BASE AbstractPolicyHolder base class
template <class BASE>
class GS2_CRYPTOPP_NO_VTABLE CFB_CipherTemplate : public BASE
{
public:
	/// \brief Apply keystream to data
	/// \param outString a buffer to write the transformed data
...
class GS2_CRYPTOPP_NO_VTABLE CFB_EncryptionTemplate : public CFB_CipherTemplate<BASE>
{
	bool IsForwardTransformation() const {return true;}
...
/// \tparam BASE AbstractPolicyHolder base class
template <class BASE = AbstractPolicyHolder<CFB_CipherAbstractPolicy, SymmetricCipher> >
class GS2_CRYPTOPP_NO_VTABLE CFB_DecryptionTemplate : public CFB_CipherTemplate<BASE>
{
...
#ifdef GS2_CRYPTOPP_MANUALLY_INSTANTIATE_TEMPLATES
...
NAMESPACE_BEGIN(detail)
NAMESPACE_BEGIN(CryptoPP)
GS2_CRYPTOPP_DLL_TEMPLATE_CLASS AbstractPolicyHolder<AdditiveCipherAbstractPolicy, SymmetricCipher>;
GS2_CRYPTOPP_DLL_TEMPLATE_CLASS AdditiveCipherTemplate<AbstractPolicyHolder<AdditiveCipherAbstractPolicy, SymmetricCipher> >;
GS2_CRYPTOPP_DLL_TEMPLATE_CLASS CFB_CipherTemplate<AbstractPolicyHolder<CFB_CipherAbstractPolicy, SymmetricCipher> >;
GS2_CRYPTOPP_DLL_TEMPLATE_CLASS CFB_EncryptionTemplate<AbstractPolicyHolder<CFB_CipherAbstractPolicy, SymmetricCipher> >;
GS2_CRYPTOPP_DLL_TEMPLATE_CLASS CFB_DecryptionTemplate<AbstractPolicyHolder<CFB_CipherAbstractPolicy, SymmetricCipher> >;
