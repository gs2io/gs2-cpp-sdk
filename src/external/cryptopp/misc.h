...
#ifndef GS2_CRYPTOPP_MISC_H
#define GS2_CRYPTOPP_MISC_H

#include "config.h"

#if !defined(GS2_CRYPTOPP_DOXYGEN_PROCESSING)

#if (GS2_CRYPTOPP_MSC_VERSION)
# pragma warning(push)
# pragma warning(disable: 4146 4514)
# if (GS2_CRYPTOPP_MSC_VERSION >= 1400)
...
#include "smartptr.h"
		...
		#define _interlockedbittestandset GS2_CRYPTOPP_DISABLED_INTRINSIC_1
		#define _interlockedbittestandreset GS2_CRYPTOPP_DISABLED_INTRINSIC_2
		#define _interlockedbittestandset64 GS2_CRYPTOPP_DISABLED_INTRINSIC_3
		#define _interlockedbittestandreset64 GS2_CRYPTOPP_DISABLED_INTRINSIC_4
		#include <intrin.h>
		#undef _interlockedbittestandset
		#undef _interlockedbittestandreset
		#undef _interlockedbittestandset64
		#undef _interlockedbittestandreset64
		#define GS2_CRYPTOPP_FAST_ROTATE(x) 1
	#elif _MSC_VER >= 1300
		#define GS2_CRYPTOPP_FAST_ROTATE(x) ((x) == 32 | (x) == 64)
	#else
		#define GS2_CRYPTOPP_FAST_ROTATE(x) ((x) == 32)
	#endif
#elif (defined(__MWERKS__) && TARGET_CPU_PPC) || \
	(defined(__GNUC__) && (defined(_ARCH_PWR2) || defined(_ARCH_PWR) || defined(_ARCH_PPC) || defined(_ARCH_PPC64) || defined(_ARCH_COM)))
	#define GS2_CRYPTOPP_FAST_ROTATE(x) ((x) == 32)
#elif defined(__GNUC__) && (GS2_CRYPTOPP_BOOL_X64 || GS2_CRYPTOPP_BOOL_X32 || GS2_CRYPTOPP_BOOL_X86)	// depend on GCC's peephole optimization to generate rotate instructions
	#define GS2_CRYPTOPP_FAST_ROTATE(x) 1
#else
	#define GS2_CRYPTOPP_FAST_ROTATE(x) 0
#endif
...
#define GS2_CRYPTOPP_BYTESWAP_AVAILABLE
#include <byteswap.h>
#endif
...
#if GS2_CRYPTOPP_DOXYGEN_PROCESSING
...
T NumericLimitsMin()
{
	GS2_CRYPTOPP_ASSERT(std::numeric_limits<T>::is_specialized);
...
#if defined(GS2_CRYPTOPP_WORD128_AVAILABLE)
template<>
gs2::detail::CryptoPP::word128 NumericLimitsMin()
{
	return 0;
}
template<>
gs2::detail::CryptoPP::word128 NumericLimitsMax()
{
	return (((gs2::detail::CryptoPP::word128)W64LIT(0xffffffffffffffff)) << 64U) | (gs2::detail::CryptoPP::word128)W64LIT(0xffffffffffffffff);
}
#endif
...
#define GS2_CRYPTOPP_COMPILE_ASSERT(expr) { ... }
#else // GS2_CRYPTOPP_DOXYGEN_PROCESSING
template <bool b>
...
#define GS2_CRYPTOPP_COMPILE_ASSERT(assertion) GS2_CRYPTOPP_COMPILE_ASSERT_INSTANCE(assertion, __LINE__)
#if defined(GS2_CRYPTOPP_EXPORTS) || defined(GS2_CRYPTOPP_IMPORTS)
#define GS2_CRYPTOPP_COMPILE_ASSERT_INSTANCE(assertion, instance)
#else
# if defined(__GNUC__)
#  define GS2_CRYPTOPP_COMPILE_ASSERT_INSTANCE(assertion, instance) \
		static CompileAssert<(assertion)> \
		GS2_CRYPTOPP_ASSERT_JOIN(cryptopp_GS2_CRYPTOPP_ASSERT_, instance) __attribute__ ((unused))
# else
#  define GS2_CRYPTOPP_COMPILE_ASSERT_INSTANCE(assertion, instance) \
		static CompileAssert<(assertion)> \
		GS2_CRYPTOPP_ASSERT_JOIN(cryptopp_GS2_CRYPTOPP_ASSERT_, instance)
# endif // __GNUC__
#endif
#define GS2_CRYPTOPP_ASSERT_JOIN(X, Y) GS2_CRYPTOPP_DO_ASSERT_JOIN(X, Y)
#define GS2_CRYPTOPP_DO_ASSERT_JOIN(X, Y) X##Y
...
class GS2_CRYPTOPP_DLL Empty
{
};

#if !defined(GS2_CRYPTOPP_DOXYGEN_PROCESSING)
template <class BASE1, class BASE2>
class GS2_CRYPTOPP_NO_VTABLE TwoBases : public BASE1, public BASE2
{
};

template <class BASE1, class BASE2, class BASE3>
class GS2_CRYPTOPP_NO_VTABLE ThreeBases : public BASE1, public BASE2, public BASE3
{
};
#endif // GS2_CRYPTOPP_DOXYGEN_PROCESSING
...
#else
#if defined(GS2_CRYPTOPP_CXX11_ATOMICS)
...
public:
	...
	GS2_CRYPTOPP_NOINLINE const T & Ref(GS2_CRYPTOPP_NOINLINE_DOTDOTDOT) const;
...
template <class T, class F, int instance>
  const T & Singleton<T, F, instance>::Ref(GS2_CRYPTOPP_NOINLINE_DOTDOTDOT) const
{
#if defined(GS2_CRYPTOPP_CXX11_ATOMICS) && defined(GS2_CRYPTOPP_CXX11_SYNCHRONIZATION) && defined(GS2_CRYPTOPP_CXX11_DYNAMIC_INIT)
...
#if (!__STDC_WANT_SECURE_LIB__ && !defined(_MEMORY_S_DEFINED)) || defined(GS2_CRYPTOPP_WANT_SECURE_LIB)
...
///   make the symbols available if <tt>GS2_CRYPTOPP_WANT_SECURE_LIB</tt> is defined.
///   <tt>GS2_CRYPTOPP_WANT_SECURE_LIB</tt> is in config.h, but it is disabled by default.
...
# define memcpy_s gs2::detail::CryptoPP::memcpy_s
# define memmove_s gs2::detail::CryptoPP::memmove_s
#endif
...
#if GS2_CRYPTOPP_GCC_VERSION >= 30001
...
# if (GS2_CRYPTOPP_LLVM_CLANG_VERSION >= 20800) || (GS2_CRYPTOPP_APPLE_CLANG_VERSION >= 30000)
#  pragma GCC diagnostic ignored "-Wtautological-compare"
# elif (GS2_CRYPTOPP_GCC_VERSION >= 40300)
...
{
	GS2_CRYPTOPP_COMPILE_ASSERT((sizeof(T1)<=sizeof(T2) && T2(-1)>0) || (sizeof(T1)>sizeof(T2) && T1(-1)>0));
...
#define GS2_CRYPTOPP_GET_BYTE_AS_BYTE(x, y) byte((x)>>(8*(y)))
...
inline unsigned int TrailingZeros(word32 v)
{
	...
	GS2_CRYPTOPP_ASSERT(v != 0);
#if defined(__BMI__)
	return (unsigned int)_tzcnt_u32(v);
#elif defined(__GNUC__) && (GS2_CRYPTOPP_GCC_VERSION >= 30400)
...
GS2_CRYPTOPP_DLL void GS2_CRYPTOPP_API xorbuf(byte *buf, const byte *mask, size_t count);
...
GS2_CRYPTOPP_DLL void GS2_CRYPTOPP_API xorbuf(byte *output, const byte *input, const byte *mask, size_t count);
...
/// \sa ModPowerOf2
GS2_CRYPTOPP_DLL bool GS2_CRYPTOPP_API VerifyBufsEqual(const byte *buf1, const byte *buf2, size_t count);
...
{
	GS2_CRYPTOPP_ASSERT(IsPowerOf2(b));
...
#if !defined(GS2_CRYPTOPP_APPLE_CLANG_VERSION) || (GS2_CRYPTOPP_APPLE_CLANG_VERSION >= 80000)
	GS2_CRYPTOPP_ASSERT(std::numeric_limits<T1>::is_integer);
	GS2_CRYPTOPP_ASSERT(std::numeric_limits<T2>::is_integer);
#endif

	GS2_CRYPTOPP_ASSERT(!std::numeric_limits<T1>::is_signed || n > 0);
	GS2_CRYPTOPP_ASSERT(!std::numeric_limits<T2>::is_signed || m > 0);
...
///   In <em>all</em> cases, if <tt>GS2_CRYPTOPP_ALLOW_UNALIGNED_DATA_ACCESS</tt> is defined, then the
///   function returns 1.
template <class T>
inline unsigned int GetAlignmentOf()
{
// GCC 4.6 (circa 2008) and above aggressively uses vectorization.
#if defined(GS2_CRYPTOPP_ALLOW_UNALIGNED_DATA_ACCESS)
	if (sizeof(T) < 16)
		return 1;
#endif

#if defined(GS2_CRYPTOPP_CXX11_ALIGNOF)
	return alignof(T);
#elif (_MSC_VER >= 1300)
	return __alignof(T);
#elif defined(__GNUC__)
	return __alignof__(T);
#elif GS2_CRYPTOPP_BOOL_SLOW_WORD64
...
#elif defined(GS2_CRYPTOPP_BIG_ENDIAN)
...
/// \details NativeByteOrder is a typedef depending on the platform. If GS2_CRYPTOPP_LITTLE_ENDIAN is
///   set in config.h, then GetNativeByteOrder returns LittleEndian. If
///   GS2_CRYPTOPP_BIG_ENDIAN is set, then GetNativeByteOrder returns BigEndian.
...
GS2_CRYPTOPP_DLL void GS2_CRYPTOPP_API CallNewHandler();
...
{
	GS2_CRYPTOPP_ASSERT(inout != NULLPTR); GS2_CRYPTOPP_ASSERT(size < INT_MAX);
...
{
	GS2_CRYPTOPP_ASSERT(output != NULLPTR); GS2_CRYPTOPP_ASSERT(input != NULLPTR); GS2_CRYPTOPP_ASSERT(size < INT_MAX);
...
#if (_MSC_VER >= 1400 || defined(__GNUC__)) && (GS2_CRYPTOPP_BOOL_X64 || GS2_CRYPTOPP_BOOL_X86)
...
#endif	// #if (_MSC_VER >= 1400 || defined(__GNUC__)) && (GS2_CRYPTOPP_BOOL_X64 || GS2_CRYPTOPP_BOOL_X86)
...
/// \note AlignedAllocate and AlignedDeallocate are available when GS2_CRYPTOPP_BOOL_ALIGN16 is
///   defined. GS2_CRYPTOPP_BOOL_ALIGN16 is defined in config.h
GS2_CRYPTOPP_DLL void* GS2_CRYPTOPP_API AlignedAllocate(size_t size);

/// \brief Frees a buffer allocated with AlignedAllocate
/// \param ptr the buffer to free
/// \note AlignedAllocate and AlignedDeallocate are available when GS2_CRYPTOPP_BOOL_ALIGN16 is
///   defined. GS2_CRYPTOPP_BOOL_ALIGN16 is defined in config.h
GS2_CRYPTOPP_DLL void GS2_CRYPTOPP_API AlignedDeallocate(void *ptr);

#endif // GS2_CRYPTOPP_DOXYGEN_PROCESSING

#if GS2_CRYPTOPP_BOOL_ALIGN16
GS2_CRYPTOPP_DLL void* GS2_CRYPTOPP_API AlignedAllocate(size_t size);
GS2_CRYPTOPP_DLL void GS2_CRYPTOPP_API AlignedDeallocate(void *ptr);
#endif // GS2_CRYPTOPP_BOOL_ALIGN16

/// \brief Allocates a buffer
/// \param size the size of the buffer
GS2_CRYPTOPP_DLL void * GS2_CRYPTOPP_API UnalignedAllocate(size_t size);

/// \brief Frees a buffer allocated with UnalignedAllocate
/// \param ptr the buffer to free
GS2_CRYPTOPP_DLL void GS2_CRYPTOPP_API UnalignedDeallocate(void *ptr);
...
template <unsigned int R, class T> inline T rotlConstant(T x)
{
	...
	GS2_CRYPTOPP_ASSERT(R < THIS_SIZE);
...
template <class T> inline T rotlFixed(T x, unsigned int y)
{
	...
	static const unsigned int THIS_SIZE = sizeof(T)*8;
	static const unsigned int MASK = THIS_SIZE-1;
	GS2_CRYPTOPP_ASSERT(y < THIS_SIZE);
...
template<> inline word32 rotlFixed<word32>(word32 x, unsigned int y)
{
	...
	GS2_CRYPTOPP_ASSERT(y < 8*sizeof(x));
...
{
#if defined(GS2_CRYPTOPP_BYTESWAP_AVAILABLE)
...
{
#if defined(__GNUC__) && defined(GS2_CRYPTOPP_X86_ASM_AVAILABLE)
	__asm__ ("bswap %0" : "=r" (value) : "0" (value));
	return value;
#elif defined(GS2_CRYPTOPP_BYTESWAP_AVAILABLE)
	return bswap_32(value);
#elif defined(__MWERKS__) && TARGET_CPU_PPC
	return (word32)__lwbrx(&value,0);
#elif (_MSC_VER >= 1400) || (defined(_MSC_VER) && !defined(_DLL))
	return _byteswap_ulong(value);
#elif GS2_CRYPTOPP_FAST_ROTATE(32) && !defined(__xlC__)
...
{
#if defined(__GNUC__) && defined(GS2_CRYPTOPP_X86_ASM_AVAILABLE) && defined(__x86_64__)
	__asm__ ("bswap %0" : "=r" (value) : "0" (value));
	return value;
#elif defined(GS2_CRYPTOPP_BYTESWAP_AVAILABLE)
...
{
#if GS2_CRYPTOPP_BOOL_SLOW_WORD64
...
inline T BitReverse(T value)
{
	...
	{
		GS2_CRYPTOPP_ASSERT(sizeof(T) == 8);
...
{
	GS2_CRYPTOPP_ASSERT(byteCount % sizeof(T) == 0);
...
inline void UnalignedbyteNonTemplate(ByteOrder order, byte *block, word16 value, const byte *xorBlock)
{
	if (order == BIG_ENDIAN_ORDER)
	{
		if (xorBlock)
		{
			block[0] = xorBlock[0] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 1);
			block[1] = xorBlock[1] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 0);
		}
		else
		{
			block[0] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 1);
			block[1] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 0);
		}
	}
	else
	{
		if (xorBlock)
		{
			block[0] = xorBlock[0] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 0);
			block[1] = xorBlock[1] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 1);
		}
		else
		{
			block[0] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 0);
			block[1] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 1);
		}
	}
}
...
inline void UnalignedbyteNonTemplate(ByteOrder order, byte *block, word32 value, const byte *xorBlock)
{
	if (order == BIG_ENDIAN_ORDER)
	{
		if (xorBlock)
		{
			block[0] = xorBlock[0] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 3);
			block[1] = xorBlock[1] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 2);
			block[2] = xorBlock[2] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 1);
			block[3] = xorBlock[3] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 0);
		}
		else
		{
			block[0] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 3);
			block[1] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 2);
			block[2] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 1);
			block[3] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 0);
		}
	}
	else
	{
		if (xorBlock)
		{
			block[0] = xorBlock[0] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 0);
			block[1] = xorBlock[1] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 1);
			block[2] = xorBlock[2] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 2);
			block[3] = xorBlock[3] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 3);
		}
		else
		{
			block[0] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 0);
			block[1] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 1);
			block[2] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 2);
			block[3] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 3);
		}
	}
}
...
inline void UnalignedbyteNonTemplate(ByteOrder order, byte *block, word64 value, const byte *xorBlock)
{
	if (order == BIG_ENDIAN_ORDER)
	{
		if (xorBlock)
		{
			block[0] = xorBlock[0] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 7);
			block[1] = xorBlock[1] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 6);
			block[2] = xorBlock[2] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 5);
			block[3] = xorBlock[3] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 4);
			block[4] = xorBlock[4] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 3);
			block[5] = xorBlock[5] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 2);
			block[6] = xorBlock[6] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 1);
			block[7] = xorBlock[7] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 0);
		}
		else
		{
			block[0] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 7);
			block[1] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 6);
			block[2] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 5);
			block[3] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 4);
			block[4] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 3);
			block[5] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 2);
			block[6] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 1);
			block[7] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 0);
		}
	}
	else
	{
		if (xorBlock)
		{
			...
			block[2] = xorBlock[2] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 2);
			block[3] = xorBlock[3] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 3);
			block[4] = xorBlock[4] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 4);
			block[5] = xorBlock[5] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 5);
			block[6] = xorBlock[6] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 6);
			block[7] = xorBlock[7] ^ GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 7);
		}
		else
		{
			block[0] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 0);
			...
			block[2] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 2);
			block[3] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 3);
			block[4] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 4);
			block[5] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 5);
			block[6] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 6);
			block[7] = GS2_CRYPTOPP_GET_BYTE_AS_BYTE(value, 7);
		}
	}
}
#endif	// #ifndef GS2_CRYPTOPP_ALLOW_UNALIGNED_DATA_ACCESS
...
{
	GS2_CRYPTOPP_UNUSED(assumeAligned);
#ifdef GS2_CRYPTOPP_ALLOW_UNALIGNED_DATA_ACCESS
...
public:
	...
	{
		GS2_CRYPTOPP_COMPILE_ASSERT(sizeof(U) >= sizeof(T));
...
template<> struct SafeShifter<true>
{
	...
	{
		GS2_CRYPTOPP_UNUSED(value); GS2_CRYPTOPP_UNUSED(bits);
		return 0;
	}
...
#ifdef GS2_CRYPTOPP_CXX11_LAMBDA
...
#define GS2_CRYPTOPP_BLOCK_1(n, t, s) t* m_##n() {return (t *)(void *)(m_aggregate+0);}     size_t SS1() {return       sizeof(t)*(s);} size_t m_##n##Size() {return (s);}
#define GS2_CRYPTOPP_BLOCK_2(n, t, s) t* m_##n() {return (t *)(void *)(m_aggregate+SS1());} size_t SS2() {return SS1()+sizeof(t)*(s);} size_t m_##n##Size() {return (s);}
#define GS2_CRYPTOPP_BLOCK_3(n, t, s) t* m_##n() {return (t *)(void *)(m_aggregate+SS2());} size_t SS3() {return SS2()+sizeof(t)*(s);} size_t m_##n##Size() {return (s);}
#define GS2_CRYPTOPP_BLOCK_4(n, t, s) t* m_##n() {return (t *)(void *)(m_aggregate+SS3());} size_t SS4() {return SS3()+sizeof(t)*(s);} size_t m_##n##Size() {return (s);}
#define GS2_CRYPTOPP_BLOCK_5(n, t, s) t* m_##n() {return (t *)(void *)(m_aggregate+SS4());} size_t SS5() {return SS4()+sizeof(t)*(s);} size_t m_##n##Size() {return (s);}
#define GS2_CRYPTOPP_BLOCK_6(n, t, s) t* m_##n() {return (t *)(void *)(m_aggregate+SS5());} size_t SS6() {return SS5()+sizeof(t)*(s);} size_t m_##n##Size() {return (s);}
#define GS2_CRYPTOPP_BLOCK_7(n, t, s) t* m_##n() {return (t *)(void *)(m_aggregate+SS6());} size_t SS7() {return SS6()+sizeof(t)*(s);} size_t m_##n##Size() {return (s);}
#define GS2_CRYPTOPP_BLOCK_8(n, t, s) t* m_##n() {return (t *)(void *)(m_aggregate+SS7());} size_t SS8() {return SS7()+sizeof(t)*(s);} size_t m_##n##Size() {return (s);}
#define GS2_CRYPTOPP_BLOCKS_END(i) size_t SST() {return SS##i();} void AllocateBlocks() {m_aggregate.New(SST());} AlignedSecByteBlock m_aggregate;
