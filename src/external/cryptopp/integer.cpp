...
#if GS2_CRYPTOPP_BOOL_X32 || defined(GS2_CRYPTOPP_DISABLE_INTEL_ASM)
# undef GS2_CRYPTOPP_X86_ASM_AVAILABLE
# undef GS2_CRYPTOPP_X32_ASM_AVAILABLE
# undef GS2_CRYPTOPP_X64_ASM_AVAILABLE
# undef GS2_CRYPTOPP_SSE2_ASM_AVAILABLE
# undef GS2_CRYPTOPP_SSSE3_ASM_AVAILABLE
#else
# define GS2_CRYPTOPP_INTEGER_SSE2 (GS2_CRYPTOPP_SSE2_ASM_AVAILABLE && (GS2_CRYPTOPP_BOOL_X86))
#endif
...
#if defined(GS2_CRYPTOPP_CXX11_SYNCHRONIZATION) && defined(GS2_CRYPTOPP_CXX11_DYNAMIC_INIT)
...
static word AtomicInverseModPower2(word A)
{
	...
	GS2_CRYPTOPP_ASSERT(R*A==1);
	return R;
}

// ********************************************************

#if !defined(GS2_CRYPTOPP_NATIVE_DWORD_AVAILABLE) || (defined(__x86_64__) && defined(GS2_CRYPTOPP_WORD128_AVAILABLE))
...
private:
    ...
    struct half_words
    {
    #ifdef GS2_CRYPTOPP_LITTLE_ENDIAN
        word low;
        word high;
    #else
        word high;
...
{
	GS2_CRYPTOPP_UNUSED(dummy);

	// Assert {A[2],A[1]} < {B1,B0}, so quotient can fit in a S
	GS2_CRYPTOPP_ASSERT(A[2] < B1 || (A[2]==B1 && A[1] < B0));
	...
	while (A[2] || A[1] > B1 || (A[1]==B1 && A[0]>=B0))
	{
		...
		A[2] += u.GetHighHalf();
		Q++;
		GS2_CRYPTOPP_ASSERT(Q);	// shouldn't overflow
	}
...
#ifdef GS2_CRYPTOPP_X64_MASM_AVAILABLE
extern "C" {
int Baseline_Add(size_t N, word *C, const word *A, const word *B);
int Baseline_Sub(size_t N, word *C, const word *A, const word *B);
}
#elif defined(GS2_CRYPTOPP_X64_ASM_AVAILABLE) && defined(__GNUC__) && defined(GS2_CRYPTOPP_WORD128_AVAILABLE)
int Baseline_Add(size_t N, word *C, const word *A, const word *B)
{
	word result;
	__asm__ __volatile__
...
#elif defined(GS2_CRYPTOPP_X86_ASM_AVAILABLE) && GS2_CRYPTOPP_BOOL_X86
GS2_CRYPTOPP_NAKED int GS2_CRYPTOPP_FASTCALL Baseline_Add(size_t N, word *C, const word *A, const word *B)
{
	AddPrologue
...
GS2_CRYPTOPP_NAKED int GS2_CRYPTOPP_FASTCALL Baseline_Sub(size_t N, word *C, const word *A, const word *B)
{
	AddPrologue
...
#if GS2_CRYPTOPP_INTEGER_SSE2
GS2_CRYPTOPP_NAKED int GS2_CRYPTOPP_FASTCALL SSE2_Add(size_t N, word *C, const word *A, const word *B)
{
	AddPrologue
...
GS2_CRYPTOPP_NAKED int GS2_CRYPTOPP_FASTCALL SSE2_Sub(size_t N, word *C, const word *A, const word *B)
{
	AddPrologue
...
#endif	// GS2_CRYPTOPP_INTEGER_SSE2
#else   // GS2_CRYPTOPP_SSE2_ASM_AVAILABLE
int GS2_CRYPTOPP_FASTCALL Baseline_Add(size_t N, word *C, const word *A, const word *B)
{
	GS2_CRYPTOPP_ASSERT (N%2 == 0);
...
int GS2_CRYPTOPP_FASTCALL Baseline_Sub(size_t N, word *C, const word *A, const word *B)
{
	GS2_CRYPTOPP_ASSERT (N%2 == 0);
...
#if !GS2_CRYPTOPP_INTEGER_SSE2	// save memory by not compiling these functions when SSE2 is available
...
#endif	// #if GS2_CRYPTOPP_INTEGER_SSE2

// ********************************************************

typedef int (GS2_CRYPTOPP_FASTCALL * PAdd)(size_t N, word *C, const word *A, const word *B);
...
void MontgomeryReduce(word *R, word *T, word *X, const word *M, const word *U, size_t N)
{
	...
	GS2_CRYPTOPP_ASSERT(carry | !borrow);
	GS2_CRYPTOPP_UNUSED(carry), GS2_CRYPTOPP_UNUSED(borrow);
...
static inline void AtomicDivide(word *Q, const word *A, const word *B)
{
	...
	else
	{
		...
		GS2_CRYPTOPP_ASSERT(!T[2] && !T[3] && (T[1] < B[1] || (T[1]==B[1] && T[0]<B[0])));
		word P[4];
		LowLevel::Multiply2(P, Q, B);
		Add(P, P, T, 4);
		GS2_CRYPTOPP_ASSERT(memcmp(P, A, 4*WORD_SIZE)==0);
#endif
	}
}
*/
...
static void CorrectQuotientEstimate(word *R, word *T, word *Q, const word *B, size_t N)
{
	...
	GS2_CRYPTOPP_ASSERT(!borrow && !R[N+1]);
	GS2_CRYPTOPP_UNUSED(borrow);

	while (R[N] || Compare(R, B, N) >= 0)
	{
		R[N] -= Subtract(R, R, B, N);
		Q[1] += (++Q[0]==0);
		GS2_CRYPTOPP_ASSERT(Q[0] || Q[1]); // no overflow
	}
}
...
{
	GS2_CRYPTOPP_ASSERT(NA && NB && NA%2==0 && NB%2==0);
	GS2_CRYPTOPP_ASSERT(B[NB-1] || B[NB-2]);
	GS2_CRYPTOPP_ASSERT(NB <= NA);
	...
	GS2_CRYPTOPP_ASSERT(shiftBits < WORD_BITS);
	...
	{
		NA+=2;
		GS2_CRYPTOPP_ASSERT(Compare(TA+NA-NB, TB, NB) < 0);
	}
...
{
	GS2_CRYPTOPP_ASSERT(NA<=N && N && N%2==0);
...
{
	GS2_CRYPTOPP_ASSERT(IsConvertableToLong());
...
{
	GS2_CRYPTOPP_ASSERT(o == BIG_ENDIAN_ORDER || o == LITTLE_ENDIAN_ORDER);
...
{
	GS2_CRYPTOPP_ASSERT(encodedInteger && byteCount); // NULL buffer
	GS2_CRYPTOPP_ASSERT(o == BIG_ENDIAN_ORDER || o == LITTLE_ENDIAN_ORDER);
...
{
	lword v = 0;
	GS2_CRYPTOPP_ASSERT(n <= sizeof(v)*8);
...
{
	GS2_CRYPTOPP_ASSERT( order == BIG_ENDIAN_ORDER || order == LITTLE_ENDIAN_ORDER );
...
{
	GS2_CRYPTOPP_ASSERT(input && inputLen); // NULL buffer
...
{
	GS2_CRYPTOPP_ASSERT(bt.MaxRetrievable() >= inputLen);
...
{
	GS2_CRYPTOPP_ASSERT(output && outputLen);            // NULL buffer
...
{
	GS2_CRYPTOPP_ASSERT(input && len); // NULL buffer
...
{
	GS2_CRYPTOPP_ASSERT(output && bufferSize);            // NULL buffer
	GS2_CRYPTOPP_ASSERT(bufferSize >= MinEncodedSize());  // Undersized buffer
...
public:
	...
	{
		GS2_CRYPTOPP_ASSERT(output && size); // NULL buffer
...
#ifdef GS2_CRYPTOPP_USE_STD_SHOWBASE
...
Integer& Integer::operator++()
{
	...
	else
	{
		...
		GS2_CRYPTOPP_ASSERT(!borrow); GS2_CRYPTOPP_UNUSED(borrow);
...
int Integer::PositiveCompare(const Integer& t) const
{
	...
	if (size != tSize)
		...
		return gs2::detail::CryptoPP::Compare(reg, t.reg, size);
}
...
{
	GS2_CRYPTOPP_ASSERT(m.NotNegative());
	GS2_CRYPTOPP_ASSERT(m.NotZero());
...
const Integer& ModularArithmetic::Half(const Integer &a) const
{
	...
	{
		gs2::detail::CryptoPP::DivideByPower2Mod(m_result.reg.begin(), a.reg, 1, m_modulus.reg, a.reg.size());
		return m_result;
	}
	else
...
const Integer& ModularArithmetic::Add(const Integer &a, const Integer &b) const
{
	...
	{
		if (gs2::detail::CryptoPP::Add(m_result.reg.begin(), a.reg, b.reg, a.reg.size())
			|| Compare(m_result.reg, m_modulus.reg, a.reg.size()) >= 0)
		{
			gs2::detail::CryptoPP::Subtract(m_result.reg.begin(), m_result.reg, m_modulus.reg, a.reg.size());
		}
		return m_result;
	}
	else
...
Integer& ModularArithmetic::Accumulate(Integer &a, const Integer &b) const
{
	if (a.reg.size()==m_modulus.reg.size() && b.reg.size()==m_modulus.reg.size())
	{
		if (gs2::detail::CryptoPP::Add(a.reg, a.reg, b.reg, a.reg.size())
			|| Compare(a.reg, m_modulus.reg, a.reg.size()) >= 0)
		{
			gs2::detail::CryptoPP::Subtract(a.reg, a.reg, m_modulus.reg, a.reg.size());
		}
	}
	else
	{
...
const Integer& ModularArithmetic::Subtract(const Integer &a, const Integer &b) const
{
	if (a.reg.size()==m_modulus.reg.size() && b.reg.size()==m_modulus.reg.size())
	{
		if (gs2::detail::CryptoPP::Subtract(m_result.reg.begin(), a.reg, b.reg, a.reg.size()))
			gs2::detail::CryptoPP::Add(m_result.reg.begin(), m_result.reg, m_modulus.reg, a.reg.size());
		return m_result;
	}
	else
	{
...
Integer& ModularArithmetic::Reduce(Integer &a, const Integer &b) const
{
	if (a.reg.size()==m_modulus.reg.size() && b.reg.size()==m_modulus.reg.size())
	{
		if (gs2::detail::CryptoPP::Subtract(a.reg, a.reg, b.reg, a.reg.size()))
			gs2::detail::CryptoPP::Add(a.reg, a.reg, m_modulus.reg, a.reg.size());
	}
	else
	{
		a-=b;
...
const Integer& ModularArithmetic::Inverse(const Integer &a) const
{
	...
	if (gs2::detail::CryptoPP::Subtract(m_result.reg.begin(), m_result.reg, a.reg, a.reg.size()))
...
const Integer& MontgomeryRepresentation::Multiply(const Integer &a, const Integer &b) const
{
	...
	GS2_CRYPTOPP_ASSERT(a.reg.size()<=N && b.reg.size()<=N);
...
const Integer& MontgomeryRepresentation::Square(const Integer &a) const
{
	...
	word *const R = m_result.reg.begin();
	const size_t N = m_modulus.reg.size();
	GS2_CRYPTOPP_ASSERT(a.reg.size()<=N);

	gs2::detail::CryptoPP::Square(T, T+2*N, a.reg, a.reg.size());
...
std::string IntToString<Integer>(Integer value, unsigned int base)
{
	...
	base &= ~(BIT_32|BIT_31);
	GS2_CRYPTOPP_ASSERT(base >= 2 && base <= 32);
...
#ifndef GS2_CRYPTOPP_NO_ASSIGN_TO_INTEGER
...
#endif  // GS2_CRYPTOPP_NO_ASSIGN_TO_INTEGER
...
// available. Dynamic initialization will be used if init priorities are not available.
	...
	const InitInteger s_init __attribute__ ((init_priority (GS2_CRYPTOPP_INIT_PRIORITY + 10))) = InitInteger();
...
{
#if defined(GS2_CRYPTOPP_CXX11_DYNAMIC_INIT)
