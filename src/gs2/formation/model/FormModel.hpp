/*
 * Copyright 2016 Game Server Services, Inc. or its affiliates. All Rights
 * Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License").
 * You may not use this file except in compliance with the License.
 * A copy of the License is located at
 *
 *  http://www.apache.org/licenses/LICENSE-2.0
 *
 * or in the "license" file accompanying this file. This file is distributed
 * on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing
 * permissions and limitations under the License.
 */

#ifndef GS2_FORMATION_MODEL_FORMMODEL_HPP_
#define GS2_FORMATION_MODEL_FORMMODEL_HPP_

#include <gs2/core/Gs2Object.hpp>
#include <gs2/core/json/IModel.hpp>
#include <gs2/core/json/JsonParser.hpp>
#include <gs2/core/util/List.hpp>
#include <gs2/core/util/StringHolder.hpp>
#include <gs2/core/util/StandardAllocator.hpp>
#include <gs2/core/external/optional/optional.hpp>
#include "SlotModel.hpp"
#include <memory>
#include <cstring>

namespace gs2 { namespace formation {

/**
 * フォームモデル
 *
 * @author Game Server Services, Inc.
 *
 */
class FormModel : public Gs2Object
{
    friend bool operator!=(const FormModel& lhs, const FormModel& lhr);

private:
    class Data : public detail::json::IModel
    {
    public:
        /** フォームマスター */
        optional<StringHolder> formModelId;
        /** フォームの種類名 */
        optional<StringHolder> name;
        /** フォームの種類のメタデータ */
        optional<StringHolder> metadata;
        /** スリットリスト */
        optional<List<SlotModel>> slots;

        Data() = default;

        Data(const Data& data) :
            detail::json::IModel(data),
            formModelId(data.formModelId),
            name(data.name),
            metadata(data.metadata)
        {
            if (data.slots)
            {
                slots = data.slots->deepCopy();
            }
        }

        Data(Data&& data) = default;

        ~Data() = default;

        Data& operator=(const Data&) = delete;
        Data& operator=(Data&&) = delete;

        virtual void set(const Char name_[], const detail::json::JsonConstValue& jsonValue)
        {
            if (std::strcmp(name_, "formModelId") == 0)
            {
                if (jsonValue.IsString())
                {
                    this->formModelId.emplace(jsonValue.GetString());
                }
            }
            else if (std::strcmp(name_, "name") == 0)
            {
                if (jsonValue.IsString())
                {
                    this->name.emplace(jsonValue.GetString());
                }
            }
            else if (std::strcmp(name_, "metadata") == 0)
            {
                if (jsonValue.IsString())
                {
                    this->metadata.emplace(jsonValue.GetString());
                }
            }
            else if (std::strcmp(name_, "slots") == 0)
            {
                if (jsonValue.IsArray())
                {
                    const auto& array = jsonValue.GetArray();
                    this->slots.emplace();
                    for (const detail::json::JsonConstValue* json = array.Begin(); json != array.End(); ++json) {
                        SlotModel item;
                        detail::json::JsonParser::parse(&item.getModel(), static_cast<detail::json::JsonConstObject>(detail::json::getObject(*json)));
                        *this->slots += std::move(item);
                    }
                }
            }
        }
    };

    GS2_CORE_SHARED_DATA_DEFINE_MEMBERS(Data, ensureData)

public:
    FormModel() = default;
    FormModel(const FormModel& formModel) = default;
    FormModel(FormModel&& formModel) = default;
    ~FormModel() = default;

    FormModel& operator=(const FormModel& formModel) = default;
    FormModel& operator=(FormModel&& formModel) = default;

    FormModel deepCopy() const
    {
        GS2_CORE_SHARED_DATA_DEEP_COPY_IMPLEMENTATION(FormModel);
    }

    const FormModel* operator->() const
    {
        return this;
    }

    FormModel* operator->()
    {
        return this;
    }
    /**
     * フォームマスターを取得
     *
     * @return フォームマスター
     */
    const optional<StringHolder>& getFormModelId() const
    {
        return ensureData().formModelId;
    }

    /**
     * フォームマスターを設定
     *
     * @param formModelId フォームマスター
     */
    void setFormModelId(StringHolder formModelId)
    {
        ensureData().formModelId.emplace(std::move(formModelId));
    }

    /**
     * フォームマスターを設定
     *
     * @param formModelId フォームマスター
     */
    FormModel& withFormModelId(StringHolder formModelId)
    {
        setFormModelId(std::move(formModelId));
        return *this;
    }

    /**
     * フォームの種類名を取得
     *
     * @return フォームの種類名
     */
    const optional<StringHolder>& getName() const
    {
        return ensureData().name;
    }

    /**
     * フォームの種類名を設定
     *
     * @param name フォームの種類名
     */
    void setName(StringHolder name)
    {
        ensureData().name.emplace(std::move(name));
    }

    /**
     * フォームの種類名を設定
     *
     * @param name フォームの種類名
     */
    FormModel& withName(StringHolder name)
    {
        setName(std::move(name));
        return *this;
    }

    /**
     * フォームの種類のメタデータを取得
     *
     * @return フォームの種類のメタデータ
     */
    const optional<StringHolder>& getMetadata() const
    {
        return ensureData().metadata;
    }

    /**
     * フォームの種類のメタデータを設定
     *
     * @param metadata フォームの種類のメタデータ
     */
    void setMetadata(StringHolder metadata)
    {
        ensureData().metadata.emplace(std::move(metadata));
    }

    /**
     * フォームの種類のメタデータを設定
     *
     * @param metadata フォームの種類のメタデータ
     */
    FormModel& withMetadata(StringHolder metadata)
    {
        setMetadata(std::move(metadata));
        return *this;
    }

    /**
     * スリットリストを取得
     *
     * @return スリットリスト
     */
    const optional<List<SlotModel>>& getSlots() const
    {
        return ensureData().slots;
    }

    /**
     * スリットリストを設定
     *
     * @param slots スリットリスト
     */
    void setSlots(List<SlotModel> slots)
    {
        ensureData().slots.emplace(std::move(slots));
    }

    /**
     * スリットリストを設定
     *
     * @param slots スリットリスト
     */
    FormModel& withSlots(List<SlotModel> slots)
    {
        setSlots(std::move(slots));
        return *this;
    }


    detail::json::IModel& getModel()
    {
        return ensureData();
    }
};

inline bool operator!=(const FormModel& lhs, const FormModel& lhr)
{
    if (lhs.m_pData != lhr.m_pData)
    {
        if (!lhs.m_pData || !lhr.m_pData)
        {
            return true;
        }
        if (lhs.m_pData->formModelId != lhr.m_pData->formModelId)
        {
            return true;
        }
        if (lhs.m_pData->name != lhr.m_pData->name)
        {
            return true;
        }
        if (lhs.m_pData->metadata != lhr.m_pData->metadata)
        {
            return true;
        }
        if (lhs.m_pData->slots != lhr.m_pData->slots)
        {
            return true;
        }
    }
    return false;
}

inline bool operator==(const FormModel& lhs, const FormModel& lhr)
{
    return !(lhs != lhr);
}

} }

#endif //GS2_FORMATION_MODEL_FORMMODEL_HPP_