...
#ifndef GS2_FORMATION_MODEL_NAMESPACE_HPP_
#define GS2_FORMATION_MODEL_NAMESPACE_HPP_
...
private:
    ...
    public:
        ...
        optional<ScriptSetting> updateMoldScript;
        /** フォームを更新するときに実行するスクリプト */
        optional<ScriptSetting> updateFormScript;
        /** ログの出力設定 */
        ...
        {
            if (data.updateMoldScript)
            {
                updateMoldScript = data.updateMoldScript->deepCopy();
            }
            if (data.updateFormScript)
            {
                updateFormScript = data.updateFormScript->deepCopy();
            }
            if (data.logSetting)
            {
        ...
        virtual void set(const Char name_[], const detail::json::JsonConstValue& jsonValue)
        {
            ...
            else if (std::strcmp(name_, "updateMoldScript") == 0)
            {
                if (jsonValue.IsObject())
                {
                    const auto& jsonObject = detail::json::getObject(jsonValue);
                    this->updateMoldScript.emplace();
                    detail::json::JsonParser::parse(&this->updateMoldScript->getModel(), jsonObject);
                }
            }
            else if (std::strcmp(name_, "updateFormScript") == 0)
            {
                if (jsonValue.IsObject())
                {
                    const auto& jsonObject = detail::json::getObject(jsonValue);
                    this->updateFormScript.emplace();
                    detail::json::JsonParser::parse(&this->updateFormScript->getModel(), jsonObject);
                }
            }
...
public:
     ...
     */
    const optional<ScriptSetting>& getUpdateMoldScript() const
    {
        return ensureData().updateMoldScript;
    }
     ...
     */
    void setUpdateMoldScript(ScriptSetting updateMoldScript)
    {
        ensureData().updateMoldScript.emplace(std::move(updateMoldScript));
    }
     ...
     * @param updateMoldScript キャパシティを更新するときに実行するスクリプト
     */
    Namespace& withUpdateMoldScript(ScriptSetting updateMoldScript)
    {
        setUpdateMoldScript(std::move(updateMoldScript));
        return *this;
    }
     ...
     */
    const optional<ScriptSetting>& getUpdateFormScript() const
    {
        return ensureData().updateFormScript;
    }
     ...
     */
    void setUpdateFormScript(ScriptSetting updateFormScript)
    {
        ensureData().updateFormScript.emplace(std::move(updateFormScript));
    }
     ...
     * @param updateFormScript フォームを更新するときに実行するスクリプト
     */
    Namespace& withUpdateFormScript(ScriptSetting updateFormScript)
    {
        setUpdateFormScript(std::move(updateFormScript));
        return *this;
    }
...
inline bool operator!=(const Namespace& lhs, const Namespace& lhr)
{
    if (lhs.m_pData != lhr.m_pData)
    {
        ...
        if (lhs.m_pData->updateMoldScript != lhr.m_pData->updateMoldScript)
        {
            return true;
        }
        if (lhs.m_pData->updateFormScript != lhr.m_pData->updateFormScript)
        {
            return true;
        }
...
#endif //GS2_FORMATION_MODEL_NAMESPACE_HPP_