...
{
public:
    constexpr static const Char* const FUNCTION = "GetTrigger";
...
public:
    ...
    {
        Gs2BasicRequest::operator=(getTriggerRequest);
    ...
    {
        Gs2BasicRequest::operator=(std::move(getTriggerRequest));
     ...
     * @param scheduleName スケジュールの名前を指定します。
     */
    GetTriggerRequest& withScheduleName(const Char* scheduleName)
    {
        ensureData().scheduleName.emplace(scheduleName);
        return *this;
    }
     ...
     * @param userId ユーザIDを指定します。
     */
    GetTriggerRequest& withUserId(const Char* userId)
    {
        ensureData().userId.emplace(userId);
        return *this;
    }
