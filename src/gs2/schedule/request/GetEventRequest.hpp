...
public:
     ...
     * @return this
     */
    GetEventRequest& withAccessToken(StringHolder accessToken)
    {
     ...
     * @param namespaceName ネームスペース名
     */
    GetEventRequest& withNamespaceName(StringHolder namespaceName)
    {
        ensureData().namespaceName.emplace(std::move(namespaceName));
        return *this;
    }
     ...
     * @param duplicationAvoider 重複実行回避機能に使用するID
     */
    GetEventRequest& withDuplicationAvoider(StringHolder duplicationAvoider)
    {
        ensureData().duplicationAvoider.emplace(std::move(duplicationAvoider));
        return *this;
    }
