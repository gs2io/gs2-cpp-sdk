...
#ifndef GS2_GATEWAY_MODEL_NAMESPACE_HPP_
#define GS2_GATEWAY_MODEL_NAMESPACE_HPP_
...
private:
    ...
    public:
        ...
        virtual void set(const Char name_[], const detail::json::JsonConstValue& jsonValue)
        {
            ...
            else if (std::strcmp(name_, "firebaseSecret") == 0)
            {
                if (jsonValue.IsString())
                {
                    this->firebaseSecret.emplace(jsonValue.GetString());
                }
            }
...
public:
     ...
     */
    void setFirebaseSecret(StringHolder firebaseSecret)
    {
     ...
     * @param firebaseSecret Firebase の通知送信に使用するシークレットトークン
     */
    Namespace& withFirebaseSecret(StringHolder firebaseSecret)
    {
        setFirebaseSecret(std::move(firebaseSecret));
        return *this;
    }
...
inline bool operator!=(const Namespace& lhs, const Namespace& lhr)
{
    if (lhs.m_pData != lhr.m_pData)
    {
        ...
        if (lhs.m_pData->firebaseSecret != lhr.m_pData->firebaseSecret)
        {
            return true;
        }
...
#endif //GS2_GATEWAY_MODEL_NAMESPACE_HPP_