...
{
public:
    constexpr static const Char* const FUNCTION = "ReadMessage";
...
public:
     ...
     * @param inboxName 受信ボックスの名前を指定します。
     */
    ReadMessageRequest& withInboxName(const Char* inboxName)
    {
        ensureData().inboxName.emplace(inboxName);
        return *this;
    }
     ...
     * @param messageId 開封するメッセージIDを指定します。
     */
    ReadMessageRequest& withMessageId(const Char* messageId)
    {
        ensureData().messageId.emplace(messageId);
        return *this;
    }
