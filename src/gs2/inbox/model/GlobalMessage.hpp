...
#ifndef GS2_INBOX_MODEL_GLOBALMESSAGE_HPP_
#define GS2_INBOX_MODEL_GLOBALMESSAGE_HPP_
 ...
 *
 */
class GlobalMessage : public Gs2Object
{
    friend bool operator!=(const GlobalMessage& lhs, const GlobalMessage& lhr);
...
public:
    ...
    GlobalMessage(const GlobalMessage& globalMessage) = default;
    GlobalMessage(GlobalMessage&& globalMessage) = default;
    ~GlobalMessage() = default;

    GlobalMessage& operator=(const GlobalMessage& globalMessage) = default;
    GlobalMessage& operator=(GlobalMessage&& globalMessage) = default;

    GlobalMessage deepCopy() const
    {
        GS2_CORE_SHARED_DATA_DEEP_COPY_IMPLEMENTATION(GlobalMessage);
    }

    const GlobalMessage* operator->() const
    {
        return this;
    }

    GlobalMessage* operator->()
    {
        return this;
    }
    /**
     ...
     * @param globalMessageId 全ユーザに向けたメッセージ
     */
    GlobalMessage& withGlobalMessageId(StringHolder globalMessageId)
    {
     ...
     * @param name 全ユーザに向けたメッセージ名
     */
    GlobalMessage& withName(StringHolder name)
    {
     ...
     * @param metadata 全ユーザに向けたメッセージの内容に相当するメタデータ
     */
    GlobalMessage& withMetadata(StringHolder metadata)
    {
     ...
     * @param expiresAt 全ユーザに向けたメッセージの有効期限
     */
    GlobalMessage& withExpiresAt(Int64 expiresAt)
    {
...
inline bool operator!=(const GlobalMessage& lhs, const GlobalMessage& lhr)
{
...
#endif //GS2_INBOX_MODEL_GLOBALMESSAGE_HPP_