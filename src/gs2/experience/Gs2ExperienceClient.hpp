...
public:
     ...
     */
    Gs2ExperienceClient(Gs2RestSession& gs2RestSession) :
     ...
     */
    Gs2ExperienceClient(Gs2RestSession& gs2RestSession, const Region& region) :
	 ...
	 * @param region アクセス先リージョン
     */
    Gs2ExperienceClient(Gs2RestSession& gs2RestSession, const Char region[]) :
        AbstractGs2ClientBase(gs2RestSession, region)
    {
    }
    ...
    void getExperienceStatus(std::function<void(AsyncGetExperienceStatusResult&)> callback, GetExperienceStatusRequest& request)
    {
        ...
        if (request.getExperienceName()) {
            ...
            url += "&" + detail::StringVariable("experienceName={value}").replace("{value}", urlSafeValue);
        }
    ...
    void getExperienceModelMaster(std::function<void(AsyncGetExperienceModelMasterResult&)> callback, GetExperienceModelMasterRequest& request)
    {
        ...
        if (request.getExperienceModelName()) {
            Char urlSafeValue[2048];
            gs2::detail::encodeUrl(urlSafeValue, *request.getExperienceModelName(), sizeof(urlSafeValue));
            url += "&" + detail::StringVariable("experienceModelName={value}").replace("{value}", urlSafeValue);
        }
    ...
    void getThresholdMaster(std::function<void(AsyncGetThresholdMasterResult&)> callback, GetThresholdMasterRequest& request)
    {
        ...
        if (request.getThresholdName()) {
            Char urlSafeValue[2048];
            gs2::detail::encodeUrl(urlSafeValue, *request.getThresholdName(), sizeof(urlSafeValue));
            url += "&" + detail::StringVariable("thresholdName={value}").replace("{value}", urlSafeValue);
        }
    ...
    void getStatusWithSignature(std::function<void(AsyncGetStatusWithSignatureResult&)> callback, GetStatusWithSignatureRequest& request)
    {
        ...
        if (request.getKeyId()) {
            ...
            url += "&" + detail::StringVariable("keyId={value}").replace("{value}", urlSafeValue);
        }
