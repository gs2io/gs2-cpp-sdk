...
/**
 * カテゴリマスターJSONのマスターデータをエクスポートします のリクエストモデル
 *
...
public:
    ...
    ExportMasterRequest& operator=(const ExportMasterRequest& exportMasterRequest)
    {
        ...
        Gs2Inventory::operator=(exportMasterRequest);
    ...
    ExportMasterRequest& operator=(ExportMasterRequest&& exportMasterRequest)
    {
        ...
        Gs2Inventory::operator=(std::move(exportMasterRequest));
     ...
     * @param categoryName カテゴリ名
     */
    ExportMasterRequest& withCategoryName(const Char* categoryName)
    {
        ensureData().categoryName.emplace(categoryName);
        return *this;
    }
