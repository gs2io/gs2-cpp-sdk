...
#ifndef GS2_DICTIONARY_CONTROL_CREATENAMESPACEREQUEST_HPP_
#define GS2_DICTIONARY_CONTROL_CREATENAMESPACEREQUEST_HPP_
 ...
 */
class CreateNamespaceRequest : public Gs2BasicRequest, public Gs2Dictionary
{
public:
...
public:
     ...
     * @param entryScript エントリー登録時に実行するスクリプト
     */
    CreateNamespaceRequest& withEntryScript(ScriptSetting entryScript)
    {
        ensureData().entryScript.emplace(std::move(entryScript));
        return *this;
    }
     ...
     * @param duplicateEntryScript 登録済みのエントリーを再度登録しようとした時に実行するスクリプト
     */
    CreateNamespaceRequest& withDuplicateEntryScript(ScriptSetting duplicateEntryScript)
    {
        ensureData().duplicateEntryScript.emplace(std::move(duplicateEntryScript));
        return *this;
    }
...
#endif //GS2_DICTIONARY_CONTROL_CREATENAMESPACEREQUEST_HPP_