 ...
 */
class LockRequest : public Gs2UserRequest, public Gs2Lock
{
public:
    constexpr static const Char* const FUNCTION = "Lock";
...
public:
    ...
    {
        Gs2UserRequest::operator=(lockRequest);
    ...
    {
        Gs2UserRequest::operator=(std::move(lockRequest));
     ...
     * @param lockPoolName ロックプールの名前を指定します。
     */
    LockRequest& withLockPoolName(const Char* lockPoolName)
    {
        ensureData().lockPoolName.emplace(lockPoolName);
        return *this;
    }
     ...
     * @param resourceName ロック解除するリソースの名前
     */
    LockRequest& withResourceName(const Char* resourceName)
    {
        ensureData().resourceName.emplace(resourceName);
        return *this;
    }
