...
public:
     ...
     * @param namespaceName カテゴリー名
     */
    UnlockRequest& withNamespaceName(const Char* namespaceName)
    {
        ensureData().namespaceName.emplace(namespaceName);
        return *this;
    }
     ...
     * @param propertyId プロパティID
     */
    UnlockRequest& withPropertyId(const Char* propertyId)
    {
        ensureData().propertyId.emplace(propertyId);
        return *this;
    }
     ...
     * @param duplicationAvoider 重複実行回避機能に使用するID
     */
    UnlockRequest& withDuplicationAvoider(const Char* duplicationAvoider)
    {
        ensureData().duplicationAvoider.emplace(duplicationAvoider);
        return *this;
    }
