...
private:
    /** インベントリモデルのリスト */
    List<EzInventoryModel> m_Items;

public:
    EzListInventoryModelsResult(const gs2::inventory::DescribeInventoryModelsResult& result)
    {
        {
            auto& list = *result.getItems();
            for (int i = 0; i < list.getCount(); ++i)
            {
                m_Items += EzInventoryModel(list[i]);
            }
        }
    }
    ...
    List<EzInventoryModel>& getItems()
    {
        return m_Items;
    }
};
