...
public:
     ...
     */
    Gs2IdentifierClient(Gs2RestSession& gs2RestSession) :
     ...
     */
    Gs2IdentifierClient(Gs2RestSession& gs2RestSession, const Region& region) :
	 ...
	 * @param region アクセス先リージョン
     */
    Gs2IdentifierClient(Gs2RestSession& gs2RestSession, const Char region[]) :
        AbstractGs2ClientBase(gs2RestSession, region)
    {
    }
    ...
    void updateUser(std::function<void(AsyncUpdateUserResult&)> callback, UpdateUserRequest& request)
    {
        ...
        if (request.getUserName()) {
            Char urlSafeValue[2048];
            gs2::detail::encodeUrl(urlSafeValue, *request.getUserName(), sizeof(urlSafeValue));
            url += "&" + detail::StringVariable("userName={value}").replace("{value}", urlSafeValue);
        }
        auto& writer = detail::json::JsonWriter::getInstance();
        writer.reset();
        writer.writeObjectStart();
    ...
    void updateSecurityPolicy(std::function<void(AsyncUpdateSecurityPolicyResult&)> callback, UpdateSecurityPolicyRequest& request)
    {
        ...
        if (request.getSecurityPolicyName()) {
            Char urlSafeValue[2048];
            gs2::detail::encodeUrl(urlSafeValue, *request.getSecurityPolicyName(), sizeof(urlSafeValue));
            url += "&" + detail::StringVariable("securityPolicyName={value}").replace("{value}", urlSafeValue);
        }
        auto& writer = detail::json::JsonWriter::getInstance();
        writer.reset();
        writer.writeObjectStart();
    ...
    void getIdentifier(std::function<void(AsyncGetIdentifierResult&)> callback, GetIdentifierRequest& request)
    {
        ...
        if (request.getClientId()) {
            Char urlSafeValue[2048];
            gs2::detail::encodeUrl(urlSafeValue, *request.getClientId(), sizeof(urlSafeValue));
            url += "&" + detail::StringVariable("clientId={value}").replace("{value}", urlSafeValue);
        }
    ...
    void detachSecurityPolicy(std::function<void(AsyncDetachSecurityPolicyResult&)> callback, DetachSecurityPolicyRequest& request)
    {
        ...
        if (request.getSecurityPolicyId()) {
            Char urlSafeValue[2048];
            gs2::detail::encodeUrl(urlSafeValue, *request.getSecurityPolicyId(), sizeof(urlSafeValue));
            url += "&" + detail::StringVariable("securityPolicyId={value}").replace("{value}", urlSafeValue);
        }
